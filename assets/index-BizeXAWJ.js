(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function a(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(i){if(i.ep)return;i.ep=!0;const n=a(i);fetch(i.href,n)}})();const H="0.1.05";let L=!1,B=[];const J=50;function ae(e,o="info"){const a=new Date().toLocaleTimeString(),s={timestamp:a,message:typeof e=="object"?JSON.stringify(e,null,2):e,type:o};B.unshift(s),B.length>J&&(B=B.slice(0,J)),L&&V(),console.log(`[${a}] ${e}`)}const se=console.log;console.log=function(...e){ae(e.join(" "),"info"),se.apply(console,e)};function q(){console.log("🐛 Toggling mobile logs, current state:",L),L=!L;let e=document.getElementById("mobile-logs-panel");L?(console.log("📱 Mostrando panel de logs móvil"),e||(e=document.createElement("div"),e.id="mobile-logs-panel",e.innerHTML=`
      ...
      `,document.body.appendChild(e),document.getElementById("clear-logs-btn").addEventListener("click",K),document.getElementById("close-logs-btn").addEventListener("click",q),console.log("📱 Panel de logs creado")),e.style.display="block",V()):(console.log("📱 Ocultando panel de logs móvil"),e&&(e.style.display="none"))}function V(){const e=document.getElementById("mobile-logs-content");e&&(e.innerHTML=B.map(o=>`
  ...
  `;const a=document.getElementById("update-notification");a&&a.remove(),document.body.appendChild(o),setTimeout(()=>{document.getElementById("update-notification")&&fe()},1e4)}function fe(){const e=document.getElementById("update-notification");e&&(e.style.transform="translateX(100%)",e.style.transition="transform 0.3s ease",setTimeout(()=>e.remove(),300))}function _(e){const o=[];return!e.name||e.name.trim().length===0?o.push("El nombre es obligatorio"):e.name.trim().length<2?o.push("El nombre debe tener al menos 2 caracteres"):e.name.trim().length>50&&o.push("El nombre no puede tener más de 50 caracteres"),e.surname&&e.surname.trim().length>50&&o.push("Los apellidos no pueden tener más de 50 caracteres"),e.phone&&e.phone.trim().length>0&&(/^[\d\s\-\+\(\)\.]{6,20}$/.test(e.phone.trim())||o.push("El teléfono debe contener solo números, espacios y caracteres válidos (6-20 caracteres)")),e.email&&e.email.trim().length>0&&(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.email.trim())?e.email.trim().length>100&&o.push("El email no puede tener más de 100 caracteres"):o.push("El email debe tener un formato válido")),o}function R(e){const o=[];return!e||e.trim().length===0?(o.push("La nota no puede estar vacía"),o):(e.trim().length>1e3&&o.push("La nota no puede tener más de 1000 caracteres"),e.trim().length<3&&o.push("La nota debe tener al menos 3 caracteres"),/^[\w\s\.\,\;\:\!\?\-\(\)\[\]\"\'\/\@\#\$\%\&\*\+\=\<\>\{\}\|\~\`\ñÑáéíóúÁÉÍÓÚüÜ]*$/.test(e)||o.push("La nota contiene caracteres no permitidos"),o)}const X="contactos_diarios";let t={contacts:ge(),selected:null,notes:"",editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,showAddNoteModal:!1,addNoteContactIndex:null,allNotesPage:1,duplicates:[],showDuplicateModal:!1,showAuthModal:!1,authMode:"login",pendingAction:null};function ge(){try{return JSON.parse(localStorage.getItem(X))||[]}catch{return[]}}function S(e){localStorage.setItem(X,JSON.stringify(e))}function m(){const e=document.querySelector("#app"),o=t.editing!==null?t.contacts[t.editing]:null,a=t.selected!==null?t.contacts[t.selected].notes||{}:{};e.innerHTML=`
    ...
    ${re({})}
  `,he(),Pe();const s=document.getElementById("show-backup-modal");s&&(s.onclick=()=>{t.showBackupModal=!0,m()});const i=document.getElementById("close-backup-modal");i&&(i.onclick=()=>{t.showBackupModal=!1,m()}),document.querySelectorAll(".add-note-contact").forEach(u=>{u.onclick=f=>{const g=Number(u.dataset.index);if(!P()){t.pendingAction={type:"addNote",contactIndex:g},W()?t.authMode="login":t.authMode="setup",t.showAuthModal=!0,m();return}t.addNoteContactIndex=g,t.showAddNoteModal=!0,m()}});const n=document.getElementById("cancel-add-note");n&&(n.onclick=()=>{t.showAddNoteModal=!1,t.addNoteContactIndex=null,m()}),document.querySelectorAll(".restore-backup-btn").forEach(u=>{u.onclick=()=>ke(u.dataset.fecha)});const d=document.getElementById("restore-local-backup");d&&(d.onclick=restaurarBackupLocal)}let G=null;function he(){const e=document.getElementById("tag-filter");e&&e.addEventListener("input",l=>{clearTimeout(G),G=setTimeout(()=>{t.tagFilter=e.value,m();const c=document.getElementById("tag-filter");c&&(c.value=t.tagFilter,c.focus(),c.setSelectionRange(t.tagFilter.length,t.tagFilter.length))},300)});const o=document.getElementById("add-contact");o&&(o.onclick=()=>{t.editing=null,t.selected=null,m(),t.editing=t.contacts.length,m()}),document.querySelectorAll(".select-contact").forEach(l=>{l.onclick=c=>{c.preventDefault(),c.stopPropagation(),j(l)&&(t.selected=Number(l.dataset.index),t.editing=null,m())}}),document.querySelectorAll(".edit-contact").forEach(l=>{l.onclick=c=>{c.preventDefault(),c.stopPropagation(),j(l)&&(t.editing=Number(l.dataset.index),t.selected=null,m())}}),document.querySelectorAll(".delete-contact").forEach(l=>{l.onclick=c=>{if(c.preventDefault(),c.stopPropagation(),!j(l))return;const r=Number(l.dataset.index),p=t.contacts[r],h=p.surname?`${p.surname}, ${p.name}`:p.name;confirm(`¿Estás seguro de eliminar el contacto "${h}"?

Esta acción no se puede deshacer.`)&&(t.contacts.splice(r,1),S(t.contacts),v("Contacto eliminado correctamente","success"),t.selected=null,m())}}),document.querySelectorAll(".pin-contact").forEach(l=>{l.onclick=c=>{if(c.preventDefault(),c.stopPropagation(),!j(l))return;const r=Number(l.dataset.index);t.contacts[r].pinned&&!confirm("¿Seguro que quieres desfijar este contacto?")||(t.contacts[r].pinned=!t.contacts[r].pinned,S(t.contacts),m())}});const a=document.getElementById("contact-form");a&&(a.onsubmit=l=>{l.preventDefault();const c=Object.fromEntries(new FormData(a)),r=_(c);if(r.length>0){v("Error de validación: "+r.join(", "),"error");return}let p=c.tags?c.tags.split(",").map(k=>k.trim()).filter(Boolean):[];delete c.tags;const h={...c};t.contacts.some((k,$)=>t.editing!==null&&$===t.editing?!1:Z(k,h))&&!confirm("Ya existe un contacto similar. ¿Deseas guardarlo de todas formas?")||(t.editing!==null&&t.editing<t.contacts.length?(t.contacts[t.editing]={...t.contacts[t.editing],...c,tags:p,lastEdited:Date.now()},(window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))&&console.log("📱 CONTACTO EDITADO:",t.contacts[t.editing].name,"Nueva fecha:",new Date().toLocaleString()),v("Contacto actualizado correctamente","success")):(t.contacts.push({...c,notes:{},tags:p,lastEdited:Date.now(),createdAt:Date.now()}),v("Contacto añadido correctamente","success")),S(t.contacts),t.editing=null,m())},document.getElementById("cancel-edit").onclick=()=>{t.editing=null,m()});const s=document.getElementById("add-note-form");if(s&&t.addNoteContactIndex!==null){const l=c=>{c.preventDefault();const r=document.getElementById("add-note-date").value,p=document.getElementById("add-note-text").value.trim();if(!r||!p){v("Por favor, selecciona una fecha y escribe una nota","warning");return}const h=R(p);if(h.length>0){v("Error en la nota: "+h.join(", "),"error");return}const E=t.addNoteContactIndex;t.contacts[E].notes||(t.contacts[E].notes={}),t.contacts[E].notes[r]?t.contacts[E].notes[r]+=`
`+p:t.contacts[E].notes[r]=p,t.contacts[E].lastEdited=Date.now(),(window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))&&console.log(`📝 MÓVIL - Nota añadida a ${t.contacts[E].name}, lastEdited: ${t.contacts[E].lastEdited}`),S(t.contacts),v("Nota añadida correctamente","success"),t.showAddNoteModal=!1,t.addNoteContactIndex=null,m()};s.onsubmit=l}const i=document.getElementById("note-form");if(i&&t.selected!==null){const l=c=>{c.preventDefault();const r=document.getElementById("note-date").value,p=document.getElementById("note-text").value.trim();if(!r||!p){v("Por favor, selecciona una fecha y escribe una nota","warning");return}const h=R(p);if(h.length>0){v("Error en la nota: "+h.join(", "),"error");return}t.contacts[t.selected].notes||(t.contacts[t.selected].notes={}),t.contacts[t.selected].notes[r]?t.contacts[t.selected].notes[r]+=`
`+p:t.contacts[t.selected].notes[r]=p,t.contacts[t.selected].lastEdited=Date.now(),S(t.contacts),v("Nota guardada correctamente","success"),document.getElementById("note-text").value="",m()};i.onsubmit=l}document.querySelectorAll(".edit-note").forEach(l=>{l.onclick=c=>{const r=l.dataset.date,p=document.getElementById("edit-note-modal"),h=document.getElementById("edit-note-text");h.value=t.contacts[t.selected].notes[r],p.style.display="block",document.getElementById("save-edit-note").onclick=()=>{const E=h.value.trim(),k=R(E);if(k.length>0){v("Error en la nota: "+k.join(", "),"error");return}t.contacts[t.selected].notes[r]=E,t.contacts[t.selected].lastEdited=Date.now(),S(t.contacts),v("Nota actualizada correctamente","success"),p.style.display="none",m()},document.getElementById("cancel-edit-note").onclick=()=>{p.style.display="none"}}}),document.querySelectorAll(".delete-note").forEach(l=>{l.onclick=c=>{const r=l.dataset.date;confirm(`¿Estás seguro de eliminar la nota del ${r}?

Esta acción no se puede deshacer.`)&&(delete t.contacts[t.selected].notes[r],S(t.contacts),v("Nota eliminada correctamente","success"),m())}}),document.getElementById("import-btn").onclick=()=>{document.getElementById("import-file").click()},document.getElementById("export-btn").onclick=()=>{document.getElementById("export-modal").style.display="flex",document.getElementById("export-vcf").onclick=()=>{Ee(),document.getElementById("export-modal").style.display="none"},document.getElementById("export-csv").onclick=()=>{we(),document.getElementById("export-modal").style.display="none"},document.getElementById("export-json").onclick=()=>{xe(),document.getElementById("export-modal").style.display="none"},document.getElementById("cancel-export").onclick=()=>{document.getElementById("export-modal").style.display="none"}},document.getElementById("import-file").onchange=async l=>{const c=l.target.files[0];if(!c)return;const r=await c.text();let p=[];if(c.name.endsWith(".vcf"))p=be(r);else if(c.name.endsWith(".csv"))p=ye(r);else if(c.name.endsWith(".json"))try{const h=JSON.parse(r);Array.isArray(h)?p=h:h&&Array.isArray(h.contacts)&&(p=h.contacts)}catch{}if(p.length){const h=[],E=[];if(p.forEach((x,A)=>{const U=_(x);U.length===0?h.push(x):E.push({index:A+1,errors:U})}),E.length>0){const x=E.map(A=>`Contacto ${A.index}: ${A.errors.join(", ")}`).join(`
`);if(!confirm(`Se encontraron ${E.length} contacto(s) con errores:

${x}

¿Deseas importar solo los contactos válidos (${h.length})?`))return}const k=x=>t.contacts.some(A=>A.name===x.name&&A.surname===x.surname&&A.phone===x.phone),$=h.filter(x=>!k(x));$.length?(t.contacts=t.contacts.concat($),S(t.contacts),v(`${$.length} contacto(s) importado(s) correctamente`,"success"),m()):v("No se han importado contactos nuevos (todos ya existen)","info")}else v("No se pudieron importar contactos del archivo seleccionado","error")};const n=document.getElementById("close-all-notes");n&&(n.onclick=()=>{t.showAllNotes=!1,t.allNotesPage=1,m()});const d=document.getElementById("show-all-notes-btn");d&&(d.onclick=()=>{if(!P()){t.pendingAction={type:"showAllNotes"},W()?t.authMode="login":t.authMode="setup",t.showAuthModal=!0,m();return}t.showAllNotes=!0,t.allNotesPage=1,m()});const u=document.getElementById("prev-notes-page");u&&(u.onclick=()=>{t.allNotesPage>1&&(t.allNotesPage--,m())});const f=document.getElementById("next-notes-page");f&&(f.onclick=()=>{let l=[];t.contacts.forEach((r,p)=>{r.notes&&Object.entries(r.notes).forEach(([h,E])=>{l.push({date:h,text:E,contact:r,contactIndex:p})})});const c=Math.max(1,Math.ceil(l.length/4));t.allNotesPage<c&&(t.allNotesPage++,m())}),document.querySelectorAll(".edit-note-link").forEach(l=>{l.onclick=c=>{c.preventDefault();const r=Number(l.dataset.contact),p=l.dataset.date;t.selected=r,t.editing=null,t.showAllNotes=!1,m(),setTimeout(()=>{const h=document.querySelector(`.edit-note[data-date="${p}"]`);h&&h.click()},50)}}),document.querySelectorAll(".share-backup-btn").forEach(l=>{l.onclick=async()=>{const c=l.dataset.fecha,p=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]").find(x=>x.fecha===c);if(!p)return alert("No se encontró la copia seleccionada.");const h=`contactos_backup_${c}.json`,E=JSON.stringify(p.datos,null,2),k=new Blob([E],{type:"application/json"}),$=document.createElement("a");if($.href=URL.createObjectURL(k),$.download=h,$.style.display="none",document.body.appendChild($),$.click(),setTimeout(()=>{URL.revokeObjectURL($.href),$.remove()},1e3),navigator.canShare&&window.File&&window.FileReader)try{const x=new File([k],h,{type:"application/json"});navigator.canShare({files:[x]})&&await navigator.share({files:[x],title:"Backup de Contactos",text:`Copia de seguridad (${c}) de ContactosDiarios`})}catch{}}});const g=document.getElementById("manage-duplicates-btn");g&&(g.onclick=()=>{t.duplicates=Se(),t.duplicates.length===0?v("No se encontraron contactos duplicados","info"):(t.showDuplicateModal=!0,m())});const y=document.getElementById("validate-contacts-btn");y&&(y.onclick=()=>{const l=[];if(t.contacts.forEach((c,r)=>{const p=_(c);if(p.length>0){const h=c.surname?`${c.surname}, ${c.name}`:c.name;l.push({index:r+1,name:h,errors:p})}}),l.length===0)v(`✅ Todos los ${t.contacts.length} contactos son válidos`,"success");else{const c=l.map(r=>`${r.index}. ${r.name}: ${r.errors.join(", ")}`).join(`
...
¿Deseas ver más detalles en la consola del navegador?`)&&console.table(l)}});const b=document.getElementById("cancel-duplicate-resolution");b&&(b.onclick=()=>{t.showDuplicateModal=!1,t.duplicates=[],m()});const w=document.getElementById("apply-resolution");w&&(w.onclick=Ae),document.querySelectorAll('input[name^="resolution-"]').forEach(l=>{l.addEventListener("change",()=>{const c=l.name.split("-")[1],r=document.getElementById(`merge-section-${c}`),p=document.getElementById(`individual-section-${c}`);l.value==="merge"?(r.style.display="block",p.style.display="none"):l.value==="select"?(r.style.display="none",p.style.display="block"):(r.style.display="none",p.style.display="none")})}),document.querySelectorAll('.resolution-option input[type="radio"]').forEach(l=>{l.addEventListener("change",()=>{const c=l.name;document.querySelectorAll(`input[name="${c}"]`).forEach(r=>{r.closest(".resolution-option").classList.remove("selected")}),l.closest(".resolution-option").classList.add("selected")})});const N=document.getElementById("unlock-notes-btn");N&&(N.onclick=()=>{t.selected!==null&&(t.pendingAction={type:"showContactNotes",contactIndex:t.selected}),W()?t.authMode="login":t.authMode="setup",t.showAuthModal=!0,m()});const M=document.getElementById("logout-btn");M&&(M.onclick=()=>{Le(),m()});const C=document.getElementById("auth-form");C&&!C.hasAttribute("data-handler-added")&&(C.setAttribute("data-handler-added","true"),C.onsubmit=l=>{l.preventDefault();const c=document.getElementById("auth-password").value.trim();if(!c){v("Por favor, introduce una contraseña","warning");return}if(t.authMode==="setup"){const r=document.getElementById("auth-password-confirm").value.trim();if(c!==r){v("Las contraseñas no coinciden","error");return}if(c.length<4){v("La contraseña debe tener al menos 4 caracteres","warning");return}Ce(c),I.isAuthenticated=!0,I.sessionExpiry=Date.now()+30*60*1e3,t.showAuthModal=!1,C.reset(),setTimeout(()=>{te()},100),m()}else Me(c)?(t.showAuthModal=!1,C.reset(),m()):(document.getElementById("auth-password").value="",document.getElementById("auth-password").focus())});const F=document.getElementById("cancel-auth");F&&(F.onclick=()=>{t.showAuthModal=!1,t.pendingAction=null,m()});const T=document.getElementById("auth-modal");if(T){T.onclick=r=>{r.target===T&&(t.showAuthModal=!1,t.pendingAction=null,m())};const l=document.getElementById("auth-password");if(l){const r=window.innerWidth<=700?300:100;setTimeout(()=>{l.focus(),window.innerWidth<=700&&l.scrollIntoView({behavior:"smooth",block:"center"})},r)}const c=r=>{r.key==="Escape"&&t.showAuthModal&&(t.showAuthModal=!1,t.pendingAction=null,m())};document.addEventListener("keydown",c)}}function be(e){const o=[],a=e.split("END:VCARD");for(const s of a){const i=/FN:([^\n]*)/.exec(s)?.[1]?.trim(),n=/N:.*;([^;\n]*)/.exec(s)?.[1]?.trim()||"",d=/TEL.*:(.+)/.exec(s)?.[1]?.trim(),u=/EMAIL.*:(.+)/.exec(s)?.[1]?.trim();i&&o.push({name:i,surname:n,phone:d||"",email:u||"",notes:{},tags:[]})}return o}function ve(e){return e.map(o=>`BEGIN:VCARD
VERSION:3.0
FN:${o.name}
N:${o.surname||""};;;;
TEL:${o.phone||""}
...
`),i+=`
Esta acción no se puede deshacer.`,!confirm(i)){v("Operación cancelada","info");return}try{let n=0,d=0;const u=[];e.forEach(b=>{if(b.type==="merge"){const w=Q(b.contacts);t.contacts.push(w),b.contacts.forEach(N=>{u.push(N.originalIndex)}),n++}else b.type==="delete"&&(b.toDelete.forEach(w=>{u.push(w)}),d+=b.toDelete.length)}),[...new Set(u)].sort((b,w)=>w-b).forEach(b=>{b<t.contacts.length&&t.contacts.splice(b,1)}),S(t.contacts);let g="Resolución completada: ";const y=[];n>0&&y.push(`${n} contacto(s) fusionado(s)`),d>0&&y.push(`${d} duplicado(s) eliminado(s)`),g+=y.join(" y "),v(g,"success"),t.showDuplicateModal=!1,t.duplicates=[],m()}catch(n){v("Error al aplicar resolución: "+n.message,"error")}}const z="contactos_diarios_auth";let I={isAuthenticated:!1,sessionExpiry:null};function ee(e){let o=0;for(let a=0;a<e.length;a++){const s=e.charCodeAt(a);o=(o<<5)-o+s,o=o&o}return o.toString()}function Ce(e){const o=ee(e);localStorage.setItem(z,o),v("Contraseña establecida correctamente","success")}function Be(e){const o=localStorage.getItem(z);return o?ee(e)===o:!1}function W(){return localStorage.getItem(z)!==null}function Me(e){return Be(e)?(I.isAuthenticated=!0,I.sessionExpiry=Date.now()+30*60*1e3,v("Autenticación exitosa","success"),setTimeout(()=>{te()},100),!0):(v("Contraseña incorrecta","error"),!1)}function P(){return I.isAuthenticated?Date.now()>I.sessionExpiry?(I.isAuthenticated=!1,I.sessionExpiry=null,!1):!0:!1}function Le(){I.isAuthenticated=!1,I.sessionExpiry=null,v("Sesión cerrada","info")}function te(){if(!t.pendingAction)return;const e=t.pendingAction;switch(t.pendingAction=null,e.type){case"showAllNotes":t.showAllNotes=!0,t.allNotesPage=1;break;case"addNote":t.addNoteContactIndex=e.contactIndex,t.showAddNoteModal=!0;break;case"showContactNotes":t.selected=e.contactIndex,t.editing=null;break}m()}function De({visible:e,mode:o="login"}){return`
  ...
  `;try{const o=await Oe();e.innerHTML=`
    ...
    `}}async function je(){console.log("🚀 Inicializando aplicación...");const e=O(),o=new Promise(s=>setTimeout(()=>s("timeout"),5e3));if(await Promise.race([e,o])==="timeout"){console.log("⏰ Timeout obteniendo versión SW, usando fallback");let s=document.getElementById("sw-version-info");s&&(s.innerHTML=`
        <p class="version-text">Service Worker v0.0.91</p>
      `)}"serviceWorker"in navigator&&(navigator.serviceWorker.addEventListener("controllerchange",()=>{console.log("🔄 Service Worker actualizado, refrescando versión..."),setTimeout(()=>O(),500)}),navigator.serviceWorker.ready.then(()=>{console.log("✅ Service Worker listo, actualizando versión..."),setTimeout(()=>O(),2e3)}).catch(s=>{console.log("❌ Error esperando SW ready:",s)}),navigator.serviceWorker.addEventListener("message",s=>{s.data&&s.data.type==="SW_UPDATED"&&(console.log("📢 Service Worker actualizado a versión:",s.data.version),me(s.data.version),O())}))}let ne=0,D=!1;function Pe(){let e=null;window.addEventListener("scroll",()=>{D=!0,clearTimeout(e),e=setTimeout(()=>{D=!1},100)},{passive:!0}),document.addEventListener("touchstart",()=>{ne=Date.now()},{passive:!0}),document.addEventListener("touchmove",()=>{D=!0},{passive:!0}),document.addEventListener("touchend",()=>{setTimeout(()=>{D=!1},100)},{passive:!0})}function j(e){if(e&&(e.classList.contains("add-note-contact")||e.classList.contains("edit-contact")||e.classList.contains("delete-contact")||e.classList.contains("pin-contact")||e.classList.contains("select-contact")))return!0;const o=Date.now()-ne;return!D&&o>150}document.addEventListener("DOMContentLoaded",()=>{console.log("=== 📱 INICIO DEBUG MÓVIL ==="),console.log("🌐 URL:",window.location.href),console.log("📱 User Agent:",navigator.userAgent),console.log("📊 Screen:",screen.width+"x"+screen.height),console.log("🖥️ Viewport:",window.innerWidth+"x"+window.innerHeight),console.log("🗂️ localStorage disponible:",!!window.localStorage),console.log("⚙️ Service Worker:","serviceWorker"in navigator),console.log("=================================");const e=document.getElementById("debug-trigger");e?(e.addEventListener("click",q),console.log("🐛 Botón de debug configurado")):console.log("❌ No se encontró el botón de debug"),Te(),m(),je(),$e(),console.log("📱 ContactosDiarios iniciado correctamente"),console.log("🆕 Nueva funcionalidad: Contactos recientemente editados"),console.log("💡 Usa Ctrl+Shift+R para limpiar cache y forzar actualización"),console.log("🔧 También disponible: window.clearCacheAndReload()")});function Te(){let e=!1;const o=Date.now();t.contacts.forEach((a,s)=>{a.lastEdited||(a.lastEdited=a.createdAt||o-(t.contacts.length-s)*1e3*60*60,e=!0),a.createdAt||(a.createdAt=a.lastEdited,e=!0)}),e&&S(t.contacts)}
