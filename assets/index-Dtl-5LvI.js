(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const p of i.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&s(p)}).observe(document,{childList:!0,subtree:!0});function n(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(a){if(a.ep)return;a.ep=!0;const i=n(a);fetch(a.href,i)}})();const K="0.1.09";let R=!1,M=[];const X=50,A=console.log;function ie(e,t="info"){const n=new Date().toLocaleTimeString(),s={timestamp:n,message:typeof e=="object"?JSON.stringify(e,null,2):e,type:t};M.unshift(s),M.length>X&&(M=M.slice(0,X)),R&&H(),A(`[${n}] ${e}`)}console.log=function(...e){ie(e.join(" "),"info"),A.apply(console,e)};function U(){A("🐛 Toggling mobile logs, current state:",R),R=!R;let e=document.getElementById("mobile-logs-panel");R?(A("📱 Mostrando panel de logs móvil"),e||(e=document.createElement("div"),e.id="mobile-logs-panel",e.innerHTML=`
      ...
      `,document.body.appendChild(e),document.getElementById("copy-logs-btn").addEventListener("click",_e),document.getElementById("clear-logs-btn").addEventListener("click",Y),document.getElementById("close-logs-btn").addEventListener("click",U),A("📱 Panel de logs creado")),e.style.display="block",H()):(A("📱 Ocultando panel de logs móvil"),e&&(e.style.display="none"))}function H(){const e=document.getElementById("mobile-logs-content");e&&(e.innerHTML=M.map(t=>`
  ...
  `).join(""))}function Y(){A("🗑️ Limpiando logs móviles"),M=[],H()}window.toggleMobileLogs=U;window.clearMobileLogs=Y;(function(){try{const t=localStorage.getItem("app_version");if(t&&t!==K){const n=localStorage.getItem("contactos_diarios"),s=localStorage.getItem("contactos_diarios_backups"),a=localStorage.getItem("contactos_diarios_backup_fecha"),i=localStorage.getItem("contactos_diarios_webdav_config");["app_version","contactos_diarios_backups","contactos_diarios_backup_fecha","contactos_diarios_webdav_config"].forEach(m=>{localStorage.removeItem(m)}),"caches"in window&&caches.keys().then(m=>{m.forEach(c=>{(c.includes("contactosdiarios")||c.includes("contactos-diarios"))&&caches.delete(c)})}),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(m=>{m.forEach(c=>{c.scope.includes(window.location.origin)&&c.unregister()})}),n&&localStorage.setItem("contactos_diarios",n),s&&localStorage.setItem("contactos_diarios_backups",s),a&&localStorage.setItem("contactos_diarios_backup_fecha",a),i&&localStorage.setItem("contactos_diarios_webdav_config",i),location.reload()}localStorage.setItem("app_version",K)}catch{}})();function ce({contacts:e,filter:t,onSelect:n,onDelete:s}){let a=t?e.filter(c=>{const r=t.toLowerCase(),h=c.notes?Object.values(c.notes).join(" ").toLowerCase():"";return c.tags?.some(b=>b.toLowerCase().includes(r))||c.name?.toLowerCase().includes(r)||c.surname?.toLowerCase().includes(r)||h.includes(r)}):e;console.log("🔄 INICIANDO ORDENACIÓN - Estado inicial:",a.length,"contactos"),a.forEach((c,r)=>{(!c.lastEdited||isNaN(Number(c.lastEdited)))&&(console.log(`⚠️ Contacto sin fecha válida: ${c.name} - lastEdited: ${c.lastEdited}`),a[r].lastEdited=0)});for(let c=0;c<2;c++)console.log(`🔄 Pasada de ordenación #${c+1}`),a=a.slice().sort((r,h)=>{const b=/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent),E=window.location.hostname==="sasogu.github.io";if(r.pinned!==h.pinned){const B=r.pinned?-1:1;return(b||E)&&console.log(`📌 Fijado: ${r.pinned?r.name:h.name} va arriba (resultado: ${B})`),B}const k=Number(r.lastEdited)||0,I=Number(h.lastEdited)||0,w=I-k;return(b||E)&&c===0&&console.log(`📅 ${r.name}(${k}) vs ${h.name}(${I}) = ${w} (${w>0?h.name:w<0?r.name:"igual"} primero)`),w});console.log("✅ ORDENACIÓN COMPLETADA"),console.log("🔍 VALIDANDO ORDEN FINAL...");const i=a.filter(c=>c.pinned),p=a.filter(c=>!c.pinned);console.log(`📌 Contactos fijados: ${i.length}`),console.log(`📄 Contactos no fijados: ${p.length}`);let m=!0;for(let c=0;c<p.length-1;c++){const r=p[c],h=p[c+1],b=Number(r.lastEdited)||0,E=Number(h.lastEdited)||0;b<E?(console.log(`❌ ERROR: ${r.name} (${b}) debería ir DESPUÉS de ${h.name} (${E})`),m=!1):console.log(`✅ OK: ${r.name} (${b}) antes que ${h.name} (${E})`)}if(m||(console.log("🔧 FORZANDO CORRECCIÓN DEL ORDEN..."),p.sort((c,r)=>{const h=Number(c.lastEdited)||0;return(Number(r.lastEdited)||0)-h}),a=[...i,...p],console.log("✅ ORDEN CORREGIDO")),(window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))&&console.log("📱 ORDEN FINAL:",a.slice(0,8).map((c,r)=>`${r+1}. ${c.pinned?"📌":"📄"} ${c.name} (${c.lastEdited?new Date(c.lastEdited).toLocaleString():"Sin fecha"})`)),window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){console.log("📱 DEBUG MÓVIL - Orden contactos:",a.slice(0,5).map(r=>`${r.pinned?"📌":"📄"} ${r.name} (${r.lastEdited?new Date(r.lastEdited).toLocaleString():"Sin fecha"})`));const c=a.filter(r=>!r.pinned);console.log("📄 DEBUG - Contactos NO fijados ordenados:",c.slice(0,5).map(r=>`${r.name} (${r.lastEdited?new Date(r.lastEdited).toLocaleString():"Sin fecha"}) [${r.lastEdited}]`));for(let r=0;r<c.length-1;r++){const h=c[r],b=c[r+1],E=Number(h.lastEdited)||0,k=Number(b.lastEdited)||0;E<k?console.log(`❌ ERROR ORDEN: ${h.name} (${E}) debería ir DESPUÉS de ${b.name} (${k})`):console.log(`✅ ORDEN OK: ${h.name} (${E}) está antes que ${b.name} (${k})`)}}return`
    <div class="contact-list">
      <h2>Contactos</h2>
      <input id="tag-filter" class="tag-filter" placeholder="Buscar nombre, apellidos, etiqueta o nota..." value="${t||""}" />
      <ul>
        ${a.length===0?'<li class="empty">Sin contactos</li>':a.map((c,r)=>{const h=Date.now(),b=24*60*60*1e3,E=c.lastEdited&&h-c.lastEdited<b,k=E&&!c.pinned?" recently-edited":"";let I="";if(c.lastEdited){const w=h-c.lastEdited,B=Math.floor(w/(60*60*1e3)),O=Math.floor(w%(60*60*1e3)/(60*1e3));B<1?I=O<1?"Ahora":`${O}m`:B<24?I=`${B}h`:I=`${Math.floor(B/24)}d`}return`
  ...
  `}function ue({contacts:e,visible:t,page:n=1}){let s=[];e.forEach((r,h)=>{r.notes&&Object.entries(r.notes).forEach(([b,E])=>{s.push({date:b,text:E,contact:r,contactIndex:h})})}),s.sort((r,h)=>h.date.localeCompare(r.date));const a=4,i=Math.max(1,Math.ceil(s.length/a)),p=Math.min(Math.max(n,1),i),m=(p-1)*a,c=s.slice(m,m+a);return`
  ...
  `;const n=document.getElementById("update-notification");n&&n.remove(),document.body.appendChild(t),setTimeout(()=>{document.getElementById("update-notification")&&fe()},1e4)}function fe(){const e=document.getElementById("update-notification");e&&(e.style.transform="translateX(100%)",e.style.transition="transform 0.3s ease",setTimeout(()=>e.remove(),300))}function q(e){const t=[];return!e.name||e.name.trim().length===0?t.push("El nombre es obligatorio"):e.name.trim().length<2?t.push("El nombre debe tener al menos 2 caracteres"):e.name.trim().length>50&&t.push("El nombre no puede tener más de 50 caracteres"),e.surname&&e.surname.trim().length>50&&t.push("Los apellidos no pueden tener más de 50 caracteres"),e.phone&&e.phone.trim().length>0&&(/^[\d\s\-\+\(\)\.]{6,20}$/.test(e.phone.trim())||t.push("El teléfono debe contener solo números, espacios y caracteres válidos (6-20 caracteres)")),e.email&&e.email.trim().length>0&&(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.email.trim())?e.email.trim().length>100&&t.push("El email no puede tener más de 100 caracteres"):t.push("El email debe tener un formato válido")),t}function F(e){const t=[];return!e||e.trim().length===0?(t.push("La nota no puede estar vacía"),t):(e.trim().length>1e3&&t.push("La nota no puede tener más de 1000 caracteres"),e.trim().length<3&&t.push("La nota debe tener al menos 3 caracteres"),/^[\w\s\.\,\;\:\!\?\-\(\)\[\]\"\'\/\@\#\$\%\&\*\+\=\<\>\{\}\|\~\`\ñÑáéíóúÁÉÍÓÚüÜ]*$/.test(e)||t.push("La nota contiene caracteres no permitidos"),t)}const V="contactos_diarios";let o={contacts:he(),selected:null,notes:"",editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,showAddNoteModal:!1,addNoteContactIndex:null,allNotesPage:1,duplicates:[],showDuplicateModal:!1,showAuthModal:!1,authMode:"login",pendingAction:null};function he(){try{return JSON.parse(localStorage.getItem(V))||[]}catch{return[]}}function C(e){if(!e||!Array.isArray(e)){console.error("❌ ERROR: Intentando guardar contactos inválidos:",e);return}if(e.length===0){const t=localStorage.getItem(V);if(t&&t!=="[]"){console.error("❌ ERROR: Intentando sobrescribir contactos existentes con array vacío. Operación cancelada."),console.log("📊 Contactos existentes:",t);return}console.log("ℹ️ Guardando array vacío (primera vez o limpieza intencional)")}console.log(`💾 Guardando ${e.length} contactos en localStorage`),localStorage.setItem(V,JSON.stringify(e))}function f(){console.log("🎨 Iniciando render...");try{const e=document.querySelector("#app");if(!e){console.error("❌ ERROR: No se encontró el elemento #app");return}console.log("✅ Elemento #app encontrado:",e);const t=o.editing!==null?o.contacts[o.editing]:null,n=o.selected!==null?o.contacts[o.selected].notes||{}:{};console.log("📊 Estado actual:",{editing:o.editing,selected:o.selected,contactsCount:o.contacts.length}),e.innerHTML=`
    ...
    ${de({})}
  `,be(),Pe();const s=document.getElementById("show-backup-modal");s&&(s.onclick=()=>{o.showBackupModal=!0,f()});const a=document.getElementById("close-backup-modal");a&&(a.onclick=()=>{o.showBackupModal=!1,f()}),document.querySelectorAll(".add-note-contact").forEach(m=>{m.onclick=c=>{const r=Number(m.dataset.index);if(!W()){o.pendingAction={type:"addNote",contactIndex:r},z()?o.authMode="login":o.authMode="setup",o.showAuthModal=!0,f();return}o.addNoteContactIndex=r,o.showAddNoteModal=!0,f()}});const i=document.getElementById("cancel-add-note");i&&(i.onclick=()=>{o.showAddNoteModal=!1,o.addNoteContactIndex=null,f()}),document.querySelectorAll(".restore-backup-btn").forEach(m=>{m.onclick=()=>Se(m.dataset.fecha)});const p=document.getElementById("restore-local-backup");p&&(p.onclick=restaurarBackupLocal),console.log("✅ Render completado exitosamente")}catch(e){console.error("❌ ERROR en render:",e),console.error("Stack trace:",e.stack);const t=document.querySelector("#app");t&&(t.innerHTML=`
      ...
      `)}}let Z=null;function be(){const e=document.getElementById("tag-filter");e&&e.addEventListener("input",d=>{clearTimeout(Z),Z=setTimeout(()=>{o.tagFilter=e.value,f();const l=document.getElementById("tag-filter");l&&(l.value=o.tagFilter,l.focus(),l.setSelectionRange(o.tagFilter.length,o.tagFilter.length))},300)});const t=document.getElementById("add-contact");t&&(t.onclick=()=>{o.editing=null,o.selected=null,f(),o.editing=o.contacts.length,f()}),document.querySelectorAll(".select-contact").forEach(d=>{d.onclick=l=>{l.preventDefault(),l.stopPropagation(),j(d)&&(o.selected=Number(d.dataset.index),o.editing=null,f())}}),document.querySelectorAll(".edit-contact").forEach(d=>{d.onclick=l=>{l.preventDefault(),l.stopPropagation(),j(d)&&(o.editing=Number(d.dataset.index),o.selected=null,f())}}),document.querySelectorAll(".delete-contact").forEach(d=>{d.onclick=l=>{if(l.preventDefault(),l.stopPropagation(),!j(d))return;const u=Number(d.dataset.index),g=o.contacts[u],v=g.surname?`${g.surname}, ${g.name}`:g.name;confirm(`¿Estás seguro de eliminar el contacto "${v}"?

Esta acción no se puede deshacer.`)&&(o.contacts.splice(u,1),C(o.contacts),y("Contacto eliminado correctamente","success"),o.selected=null,f())}}),document.querySelectorAll(".pin-contact").forEach(d=>{d.onclick=l=>{if(l.preventDefault(),l.stopPropagation(),!j(d))return;const u=Number(d.dataset.index);o.contacts[u].pinned&&!confirm("¿Seguro que quieres desfijar este contacto?")||(o.contacts[u].pinned=!o.contacts[u].pinned,C(o.contacts),f())}});const n=document.getElementById("contact-form");n&&(n.onsubmit=d=>{d.preventDefault();const l=Object.fromEntries(new FormData(n)),u=q(l);if(u.length>0){y("Error de validación: "+u.join(", "),"error");return}let g=l.tags?l.tags.split(",").map(N=>N.trim()).filter(Boolean):[];delete l.tags;const v={...l};o.contacts.some((N,S)=>o.editing!==null&&S===o.editing?!1:ee(N,v))&&!confirm("Ya existe un contacto similar. ¿Deseas guardarlo de todas formas?")||(o.editing!==null&&o.editing<o.contacts.length?(o.contacts[o.editing]={...o.contacts[o.editing],...l,tags:g,lastEdited:Date.now()},(window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))&&console.log("📱 CONTACTO EDITADO:",o.contacts[o.editing].name,"Nueva fecha:",new Date().toLocaleString()),y("Contacto actualizado correctamente","success")):(o.contacts.push({...l,notes:{},tags:g,lastEdited:Date.now(),createdAt:Date.now()}),y("Contacto añadido correctamente","success")),C(o.contacts),o.editing=null,f())},document.getElementById("cancel-edit").onclick=()=>{o.editing=null,f()});const s=document.getElementById("add-note-form");if(s&&o.addNoteContactIndex!==null){const d=l=>{l.preventDefault();const u=document.getElementById("add-note-date").value,g=document.getElementById("add-note-text").value.trim();if(!u||!g){y("Por favor, selecciona una fecha y escribe una nota","warning");return}const v=F(g);if(v.length>0){y("Error en la nota: "+v.join(", "),"error");return}const x=o.addNoteContactIndex;o.contacts[x].notes||(o.contacts[x].notes={}),o.contacts[x].notes[u]?o.contacts[x].notes[u]+=`
`+g:o.contacts[x].notes[u]=g,o.contacts[x].lastEdited=Date.now(),(window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))&&console.log(`📝 MÓVIL - Nota añadida a ${o.contacts[x].name}, lastEdited: ${o.contacts[x].lastEdited}`),C(o.contacts),y("Nota añadida correctamente","success"),o.showAddNoteModal=!1,o.addNoteContactIndex=null,f()};s.onsubmit=d}const a=document.getElementById("note-form");if(a&&o.selected!==null){const d=l=>{l.preventDefault();const u=document.getElementById("note-date").value,g=document.getElementById("note-text").value.trim();if(!u||!g){y("Por favor, selecciona una fecha y escribe una nota","warning");return}const v=F(g);if(v.length>0){y("Error en la nota: "+v.join(", "),"error");return}o.contacts[o.selected].notes||(o.contacts[o.selected].notes={}),o.contacts[o.selected].notes[u]?o.contacts[o.selected].notes[u]+=`
`+g:o.contacts[o.selected].notes[u]=g,o.contacts[o.selected].lastEdited=Date.now(),C(o.contacts),y("Nota guardada correctamente","success"),document.getElementById("note-text").value="",f()};a.onsubmit=d}document.querySelectorAll(".edit-note").forEach(d=>{d.onclick=l=>{const u=d.dataset.date,g=document.getElementById("edit-note-modal"),v=document.getElementById("edit-note-text");v.value=o.contacts[o.selected].notes[u],g.style.display="block",document.getElementById("save-edit-note").onclick=()=>{const x=v.value.trim(),N=F(x);if(N.length>0){y("Error en la nota: "+N.join(", "),"error");return}o.contacts[o.selected].notes[u]=x,o.contacts[o.selected].lastEdited=Date.now(),C(o.contacts),y("Nota actualizada correctamente","success"),g.style.display="none",f()},document.getElementById("cancel-edit-note").onclick=()=>{g.style.display="none"}}}),document.querySelectorAll(".delete-note").forEach(d=>{d.onclick=l=>{const u=d.dataset.date;confirm(`¿Estás seguro de eliminar la nota del ${u}?

Esta acción no se puede deshacer.`)&&(delete o.contacts[o.selected].notes[u],C(o.contacts),y("Nota eliminada correctamente","success"),f())}}),document.getElementById("import-btn").onclick=()=>{document.getElementById("import-file").click()},document.getElementById("export-btn").onclick=()=>{document.getElementById("export-modal").style.display="flex",document.getElementById("export-vcf").onclick=()=>{xe(),document.getElementById("export-modal").style.display="none"},document.getElementById("export-csv").onclick=()=>{we(),document.getElementById("export-modal").style.display="none"},document.getElementById("export-json").onclick=()=>{$e(),document.getElementById("export-modal").style.display="none"},document.getElementById("cancel-export").onclick=()=>{document.getElementById("export-modal").style.display="none"}},document.getElementById("import-file").onchange=async d=>{const l=d.target.files[0];if(!l)return;const u=await l.text();let g=[];if(l.name.endsWith(".vcf"))g=ve(u);else if(l.name.endsWith(".csv"))g=Ee(u);else if(l.name.endsWith(".json"))try{const v=JSON.parse(u);Array.isArray(v)?g=v:v&&Array.isArray(v.contacts)&&(g=v.contacts)}catch{}if(g.length){const v=[],x=[];if(g.forEach(($,L)=>{const J=q($);J.length===0?v.push($):x.push({index:L+1,errors:J})}),x.length>0){const $=x.map(L=>`Contacto ${L.index}: ${L.errors.join(", ")}`).join(`
`);if(!confirm(`Se encontraron ${x.length} contacto(s) con errores:

${$}

¿Deseas importar solo los contactos válidos (${v.length})?`))return}const N=$=>o.contacts.some(L=>L.name===$.name&&L.surname===$.surname&&L.phone===$.phone),S=v.filter($=>!N($));S.length?(o.contacts=o.contacts.concat(S),C(o.contacts),y(`${S.length} contacto(s) importado(s) correctamente`,"success"),f()):y("No se han importado contactos nuevos (todos ya existen)","info")}else y("No se pudieron importar contactos del archivo seleccionado","error")};const i=document.getElementById("close-all-notes");i&&(i.onclick=()=>{o.showAllNotes=!1,o.allNotesPage=1,f()});const p=document.getElementById("show-all-notes-btn");p&&(p.onclick=()=>{if(!W()){o.pendingAction={type:"showAllNotes"},z()?o.authMode="login":o.authMode="setup",o.showAuthModal=!0,f();return}o.showAllNotes=!0,o.allNotesPage=1,f()});const m=document.getElementById("prev-notes-page");m&&(m.onclick=()=>{o.allNotesPage>1&&(o.allNotesPage--,f())});const c=document.getElementById("next-notes-page");c&&(c.onclick=()=>{let d=[];o.contacts.forEach((u,g)=>{u.notes&&Object.entries(u.notes).forEach(([v,x])=>{d.push({date:v,text:x,contact:u,contactIndex:g})})});const l=Math.max(1,Math.ceil(d.length/4));o.allNotesPage<l&&(o.allNotesPage++,f())}),document.querySelectorAll(".edit-note-link").forEach(d=>{d.onclick=l=>{l.preventDefault();const u=Number(d.dataset.contact),g=d.dataset.date;o.selected=u,o.editing=null,o.showAllNotes=!1,f(),setTimeout(()=>{const v=document.querySelector(`.edit-note[data-date="${g}"]`);v&&v.click()},50)}}),document.querySelectorAll(".share-backup-btn").forEach(d=>{d.onclick=async()=>{const l=d.dataset.fecha,g=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]").find($=>$.fecha===l);if(!g)return alert("No se encontró la copia seleccionada.");const v=`contactos_backup_${l}.json`,x=JSON.stringify(g.datos,null,2),N=new Blob([x],{type:"application/json"}),S=document.createElement("a");if(S.href=URL.createObjectURL(N),S.download=v,S.style.display="none",document.body.appendChild(S),S.click(),setTimeout(()=>{URL.revokeObjectURL(S.href),S.remove()},1e3),navigator.canShare&&window.File&&window.FileReader)try{const $=new File([N],v,{type:"application/json"});navigator.canShare({files:[$]})&&await navigator.share({files:[$],title:"Backup de Contactos",text:`Copia de seguridad (${l}) de ContactosDiarios`})}catch{}}});const r=document.getElementById("manage-duplicates-btn");r&&(r.onclick=()=>{o.duplicates=Ie(),o.duplicates.length===0?y("No se encontraron contactos duplicados","info"):(o.showDuplicateModal=!0,f())});const h=document.getElementById("validate-contacts-btn");h&&(h.onclick=()=>{const d=[];if(o.contacts.forEach((l,u)=>{const g=q(l);if(g.length>0){const v=l.surname?`${l.surname}, ${l.name}`:l.name;d.push({index:u+1,name:v,errors:g})}}),d.length===0)y(`✅ Todos los ${o.contacts.length} contactos son válidos`,"success");else{const l=d.map(u=>`${u.index}. ${u.name}: ${u.errors.join(", ")}`).join(`
...
${c}`:t.notes[m]=c})}),t.pinned=e.some(i=>i.pinned),t}function Ne(e){const t=te(e);return`
  ...
  `}function Ae(){const e=[];let t=0;if(o.duplicates.forEach((i,p)=>{const m=document.querySelector(`input[name="resolution-${p}"]:checked`),c=m?m.value:"skip";if(c==="merge")e.push({type:"merge",groupIndex:p,contacts:i.contacts}),t++;else if(c==="select"){const r=document.querySelector(`input[name="keep-${p}"]:checked`);if(r){const h=parseInt(r.value),b=i.contacts.filter(E=>E.originalIndex!==h).map(E=>E.originalIndex);e.push({type:"delete",groupIndex:p,toDelete:b,toKeep:h}),t++}}}),t===0){y("No hay operaciones que realizar","info");return}const n=e.filter(i=>i.type==="merge").length,s=e.filter(i=>i.type==="delete").length;let a=`¿Confirmar las siguientes operaciones?

`;if(n>0&&(a+=`🔗 Fusionar ${n} grupo(s) de contactos
`),s>0&&(a+=`🗑️ Eliminar duplicados en ${s} grupo(s)
`),a+=`
Esta acción no se puede deshacer.`,!confirm(a)){y("Operación cancelada","info");return}try{let i=0,p=0;const m=[];e.forEach(b=>{if(b.type==="merge"){const E=te(b.contacts);o.contacts.push(E),b.contacts.forEach(k=>{m.push(k.originalIndex)}),i++}else b.type==="delete"&&(b.toDelete.forEach(E=>{m.push(E)}),p+=b.toDelete.length)}),[...new Set(m)].sort((b,E)=>E-b).forEach(b=>{b<o.contacts.length&&o.contacts.splice(b,1)}),C(o.contacts);let r="Resolución completada: ";const h=[];i>0&&h.push(`${i} contacto(s) fusionado(s)`),p>0&&h.push(`${p} duplicado(s) eliminado(s)`),r+=h.join(" y "),y(r,"success"),o.showDuplicateModal=!1,o.duplicates=[],f()}catch(i){y("Error al aplicar resolución: "+i.message,"error")}}const G="contactos_diarios_auth";let D={isAuthenticated:!1,sessionExpiry:null};function oe(e){let t=0;for(let n=0;n<e.length;n++){const s=e.charCodeAt(n);t=(t<<5)-t+s,t=t&t}return t.toString()}function Be(e){const t=oe(e);localStorage.setItem(G,t),y("Contraseña establecida correctamente","success")}function De(e){const t=localStorage.getItem(G);return t?oe(e)===t:!1}function z(){return localStorage.getItem(G)!==null}function Le(e){return De(e)?(D.isAuthenticated=!0,D.sessionExpiry=Date.now()+30*60*1e3,y("Autenticación exitosa","success"),setTimeout(()=>{ne()},100),!0):(y("Contraseña incorrecta","error"),!1)}function W(){return D.isAuthenticated?Date.now()>D.sessionExpiry?(D.isAuthenticated=!1,D.sessionExpiry=null,!1):!0:!1}function Me(){D.isAuthenticated=!1,D.sessionExpiry=null,y("Sesión cerrada","info")}function ne(){if(!o.pendingAction)return;const e=o.pendingAction;switch(o.pendingAction=null,e.type){case"showAllNotes":o.showAllNotes=!0,o.allNotesPage=1;break;case"addNote":o.addNoteContactIndex=e.contactIndex,o.showAddNoteModal=!0;break;case"showContactNotes":o.selected=e.contactIndex,o.editing=null;break}f()}function Oe({visible:e,mode:t="login"}){return`
