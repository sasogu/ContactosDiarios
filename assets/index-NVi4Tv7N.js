(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function a(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(i){if(i.ep)return;i.ep=!0;const n=a(i);fetch(i.href,n)}})();const J="0.1.07";let D=!1,M=[];const G=50,B=console.log;function se(e,o="info"){const a=new Date().toLocaleTimeString(),s={timestamp:a,message:typeof e=="object"?JSON.stringify(e,null,2):e,type:o};M.unshift(s),M.length>G&&(M=M.slice(0,G)),D&&z(),B(`[${a}] ${e}`)}console.log=function(...e){se(e.join(" "),"info"),B.apply(console,e)};function V(){B("🐛 Toggling mobile logs, current state:",D),D=!D;let e=document.getElementById("mobile-logs-panel");D?(B("📱 Mostrando panel de logs móvil"),e||(e=document.createElement("div"),e.id="mobile-logs-panel",e.innerHTML=`
      ...
      `,document.body.appendChild(e),document.getElementById("clear-logs-btn").addEventListener("click",X),document.getElementById("close-logs-btn").addEventListener("click",V),B("📱 Panel de logs creado")),e.style.display="block",z()):(B("📱 Ocultando panel de logs móvil"),e&&(e.style.display="none"))}function z(){const e=document.getElementById("mobile-logs-content");e&&(e.innerHTML=M.map(o=>`
  ...
  `).join(""))}function X(){B("🗑️ Limpiando logs móviles"),M=[],z()}window.toggleMobileLogs=V;window.clearMobileLogs=X;(function(){try{const o=localStorage.getItem("app_version");if(o&&o!==J){const a=localStorage.getItem("contactos_diarios"),s=localStorage.getItem("contactos_diarios_backups"),i=localStorage.getItem("contactos_diarios_backup_fecha"),n=localStorage.getItem("contactos_diarios_webdav_config");["app_version","contactos_diarios","contactos_diarios_backups","contactos_diarios_backup_fecha","contactos_diarios_webdav_config"].forEach(u=>{u!=="contactos_diarios"&&localStorage.removeItem(u)}),"caches"in window&&caches.keys().then(u=>{u.forEach(g=>{(g.includes("contactosdiarios")||g.includes("contactos-diarios"))&&caches.delete(g)})}),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(u=>{u.forEach(g=>{g.scope.includes(window.location.origin)&&g.unregister()})}),a&&localStorage.setItem("contactos_diarios",a),s&&localStorage.setItem("contactos_diarios_backups",s),i&&localStorage.setItem("contactos_diarios_backup_fecha",i),n&&localStorage.setItem("contactos_diarios_webdav_config",n),location.reload()}localStorage.setItem("app_version",J)}catch{}})();function ie({contacts:e,filter:o,onSelect:a,onDelete:s}){let i=o?e.filter(n=>{const d=o.toLowerCase(),u=n.notes?Object.values(n.notes).join(" ").toLowerCase():"";return n.tags?.some(g=>g.toLowerCase().includes(d))||n.name?.toLowerCase().includes(d)||n.surname?.toLowerCase().includes(d)||u.includes(d)}):e;return i=i.slice().sort((n,d)=>{const u=/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent),g=window.location.hostname==="sasogu.github.io";if((u||g)&&console.log(`🔄 Comparando: ${n.name}(pin:${!!n.pinned}, edit:${n.lastEdited||0}) vs ${d.name}(pin:${!!d.pinned}, edit:${d.lastEdited||0})`),d.pinned&&!n.pinned)return(u||g)&&console.log(`📌 ${d.name} es fijado, va arriba`),1;if(n.pinned&&!d.pinned)return(u||g)&&console.log(`📌 ${n.name} es fijado, va arriba`),-1;if(n.pinned===d.pinned){const f=parseInt(n.lastEdited)||0,y=parseInt(d.lastEdited)||0,b=y-f;return(u||g)&&(console.log(`📅 Comparación fechas: ${y} - ${f} = ${b}`),console.log(`📅 Resultado: ${b>0?d.name:b<0?n.name:"igual"} va primero`)),b}return 0}),(window.location.hostname==="sasogu.github.io"||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))&&console.log("📱 DEBUG MÓVIL - Orden contactos:",i.slice(0,5).map(n=>`${n.pinned?"📌":"📄"} ${n.name} (${n.lastEdited?new Date(n.lastEdited).toLocaleString():"Sin fecha"})`)),`
    <div class="contact-list">
      <h2>Contactos</h2>
      <input id="tag-filter" class="tag-filter" placeholder="Buscar nombre, apellidos, etiqueta o nota..." value="${o||""}" />
      <ul>
        ${i.length===0?'<li class="empty">Sin contactos</li>':i.map((n,d)=>{const u=Date.now(),g=24*60*60*1e3,f=n.lastEdited&&u-n.lastEdited<g,y=f&&!n.pinned?" recently-edited":"";let b="";if(n.lastEdited){const x=u-n.lastEdited,N=Math.floor(x/(60*60*1e3)),L=Math.floor(x%(60*60*1e3)/(60*1e3));N<1?b=L<1?"Ahora":`${L}m`:N<24?b=`${N}h`:b=`${Math.floor(N/24)}d`}return`
  ...
  `}function de({contacts:e,visible:o,page:a=1}){let s=[];e.forEach((f,y)=>{f.notes&&Object.entries(f.notes).forEach(([b,x])=>{s.push({date:b,text:x,contact:f,contactIndex:y})})}),s.sort((f,y)=>y.date.localeCompare(f.date));const i=4,n=Math.max(1,Math.ceil(s.length/i)),d=Math.min(Math.max(a,1),n),u=(d-1)*i,g=s.slice(u,u+i);return`
  ...
  `;const a=document.getElementById("update-notification");a&&a.remove(),document.body.appendChild(o),setTimeout(()=>{document.getElementById("update-notification")&&ge()},1e4)}function ge(){const e=document.getElementById("update-notification");e&&(e.style.transform="translateX(100%)",e.style.transition="transform 0.3s ease",setTimeout(()=>e.remove(),300))}function _(e){const o=[];return!e.name||e.name.trim().length===0?o.push("El nombre es obligatorio"):e.name.trim().length<2?o.push("El nombre debe tener al menos 2 caracteres"):e.name.trim().length>50&&o.push("El nombre no puede tener más de 50 caracteres"),e.surname&&e.surname.trim().length>50&&o.push("Los apellidos no pueden tener más de 50 caracteres"),e.phone&&e.phone.trim().length>0&&(/^[\d\s\-\+\(\)\.]{6,20}$/.test(e.phone.trim())||o.push("El teléfono debe contener solo números, espacios y caracteres válidos (6-20 caracteres)")),e.email&&e.email.trim().length>0&&(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.email.trim())?e.email.trim().length>100&&o.push("El email no puede tener más de 100 caracteres"):o.push("El email debe tener un formato válido")),o}function W(e){const o=[];return!e||e.trim().length===0?(o.push("La nota no puede estar vacía"),o):(e.trim().length>1e3&&o.push("La nota no puede tener más de 1000 caracteres"),e.trim().length<3&&o.push("La nota debe tener al menos 3 caracteres"),/^[\w\s\.\,\;\:\!\?\-\(\)\[\]\"\'\/\@\#\$\%\&\*\+\=\<\>\{\}\|\~\`\ñÑáéíóúÁÉÍÓÚüÜ]*$/.test(e)||o.push("La nota contiene caracteres no permitidos"),o)}const Z="contactos_diarios";let t={contacts:fe(),selected:null,notes:"",editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,showAddNoteModal:!1,addNoteContactIndex:null,allNotesPage:1,duplicates:[],showDuplicateModal:!1,showAuthModal:!1,authMode:"login",pendingAction:null};function fe(){try{return JSON.parse(localStorage.getItem(Z))||[]}catch{return[]}}function S(e){localStorage.setItem(Z,JSON.stringify(e))}function m(){console.log("🎨 Iniciando render...");try{const e=document.querySelector("#app");if(!e){console.error("❌ ERROR: No se encontró el elemento #app");return}console.log("✅ Elemento #app encontrado:",e);const o=t.editing!==null?t.contacts[t.editing]:null,a=t.selected!==null?t.contacts[t.selected].notes||{}:{};console.log("📊 Estado actual:",{editing:t.editing,selected:t.selected,contactsCount:t.contacts.length}),e.innerHTML=`
    ...
    ${le({})}
  `,he(),Te();const s=document.getElementById("show-backup-modal");s&&(s.onclick=()=>{t.showBackupModal=!0,m()});const i=document.getElementById("close-backup-modal");i&&(i.onclick=()=>{t.showBackupModal=!1,m()}),document.querySelectorAll(".add-note-contact").forEach(u=>{u.onclick=g=>{const f=Number(u.dataset.index);if(!P()){t.pendingAction={type:"addNote",contactIndex:f},q()?t.authMode="login":t.authMode="setup",t.showAuthModal=!0,m();return}t.addNoteContactIndex=f,t.showAddNoteModal=!0,m()}});const n=document.getElementById("cancel-add-note");n&&(n.onclick=()=>{t.showAddNoteModal=!1,t.addNoteContactIndex=null,m()}),document.querySelectorAll(".restore-backup-btn").forEach(u=>{u.onclick=()=>$e(u.dataset.fecha)});const d=document.getElementById("restore-local-backup");d&&(d.onclick=restaurarBackupLocal),console.log("✅ Render completado exitosamente")}catch(e){console.error("❌ ERROR en render:",e),console.error("Stack trace:",e.stack);const o=document.querySelector("#app");o&&(o.innerHTML=`
...
`).filter(Boolean),[a,...s]=o;return s.map(i=>{const[n,d,u,g,f,y]=i.split(",");return{name:n?.trim()||"",surname:d?.trim()||"",phone:u?.trim()||"",email:g?.trim()||"",notes:y?JSON.parse(y):{},tags:f?f.split(";").map(b=>b.trim()):[]}})}function Ee(){const e=ve(t.contacts),o=new Blob([e],{type:"text/vcard"}),a=document.createElement("a");a.href=URL.createObjectURL(o),a.download="contactos.vcf",a.click()}function xe(){const e="name,surname,phone,email,tags,notes",o=t.contacts.map(n=>[n.name,n.surname,n.phone,n.email,(n.tags||[]).join(";"),JSON.stringify(n.notes||{})].map(d=>'"'+String(d).replace(/"/g,'""')+'"').join(",")),a=[e,...o].join(`
`),s=new Blob([a],{type:"text/csv"}),i=document.createElement("a");i.href=URL.createObjectURL(s),i.download="contactos.csv",i.click()}function we(){const e=new Blob([JSON.stringify(t.contacts,null,2)],{type:"application/json"}),o=document.createElement("a");o.href=URL.createObjectURL(e),o.download="contactos.json",o.click()}function Y(){const e=new Date,a=new Date(e.toLocaleString("en-US",{timeZone:"Europe/Madrid"})).toISOString().slice(0,10);let s=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]");s.find(i=>i.fecha===a)||(s.push({fecha:a,datos:t.contacts}),s.length>10&&(s=s.slice(-10)),localStorage.setItem("contactos_diarios_backups",JSON.stringify(s))),localStorage.setItem("contactos_diarios_backup_fecha",a)}setInterval(Y,60*60*1e3);Y();function ke(){const e=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]"),o=document.getElementById("backup-info");o&&(e.length===0?o.textContent="Sin copias locales.":o.innerHTML="Últimas copias locales: "+e.map(a=>`<button class="restore-backup-btn" data-fecha="${a.fecha}">${a.fecha}</button>`).join(" "))}function $e(e){if(!confirm("¿Seguro que quieres restaurar la copia de seguridad del "+e+"? Se sobrescribirán los contactos actuales."))return;const a=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]").find(s=>s.fecha===e);a?(t.contacts=a.datos,S(t.contacts),m(),alert("Backup restaurado correctamente.")):alert("No se encontró la copia seleccionada.")}function Q(e,o){const a=s=>s?s.toLowerCase().replace(/\s+/g," ").trim():"";return!!(a(e.name)===a(o.name)&&a(e.surname)===a(o.surname)||e.phone&&o.phone&&e.phone.replace(/\s+/g,"")===o.phone.replace(/\s+/g,"")||e.email&&o.email&&a(e.email)===a(o.email))}function Se(){const e=[],o=new Set;for(let a=0;a<t.contacts.length;a++){if(o.has(a))continue;const s=[{...t.contacts[a],originalIndex:a}];o.add(a);for(let i=a+1;i<t.contacts.length;i++)o.has(i)||Q(t.contacts[a],t.contacts[i])&&(s.push({...t.contacts[i],originalIndex:i}),o.add(i));s.length>1&&e.push({contacts:s})}return e}function ee(e){if(e.length===0)return null;if(e.length===1)return e[0];const o={name:"",surname:"",phone:"",email:"",tags:[],notes:{},pinned:!1};let a="",s="";e.forEach(n=>{n.name&&n.name.length>a.length&&(a=n.name),n.surname&&n.surname.length>s.length&&(s=n.surname)}),o.name=a,o.surname=s,o.phone=e.find(n=>n.phone)?.phone||"",o.email=e.find(n=>n.email)?.email||"";const i=new Set;return e.forEach(n=>{n.tags&&n.tags.forEach(d=>i.add(d))}),o.tags=Array.from(i),e.forEach((n,d)=>{n.notes&&Object.entries(n.notes).forEach(([u,g])=>{o.notes[u]?o.notes[u]+=`
--- Contacto ${d+1} ---
${g}`:o.notes[u]=g})}),o.pinned=e.some(n=>n.pinned),o}function Ie(e){const o=ee(e);return`
...
`),i+=`
Esta acción no se puede deshacer.`,!confirm(i)){v("Operación cancelada","info");return}try{let n=0,d=0;const u=[];e.forEach(b=>{if(b.type==="merge"){const x=ee(b.contacts);t.contacts.push(x),b.contacts.forEach(N=>{u.push(N.originalIndex)}),n++}else b.type==="delete"&&(b.toDelete.forEach(x=>{u.push(x)}),d+=b.toDelete.length)}),[...new Set(u)].sort((b,x)=>x-b).forEach(b=>{b<t.contacts.length&&t.contacts.splice(b,1)}),S(t.contacts);let f="Resolución completada: ";const y=[];n>0&&y.push(`${n} contacto(s) fusionado(s)`),d>0&&y.push(`${d} duplicado(s) eliminado(s)`),f+=y.join(" y "),v(f,"success"),t.showDuplicateModal=!1,t.duplicates=[],m()}catch(n){v("Error al aplicar resolución: "+n.message,"error")}}const F="contactos_diarios_auth";let I={isAuthenticated:!1,sessionExpiry:null};function te(e){let o=0;for(let a=0;a<e.length;a++){const s=e.charCodeAt(a);o=(o<<5)-o+s,o=o&o}return o.toString()}function Ce(e){const o=te(e);localStorage.setItem(F,o),v("Contraseña establecida correctamente","success")}function Be(e){const o=localStorage.getItem(F);return o?te(e)===o:!1}function q(){return localStorage.getItem(F)!==null}function Me(e){return Be(e)?(I.isAuthenticated=!0,I.sessionExpiry=Date.now()+30*60*1e3,v("Autenticación exitosa","success"),setTimeout(()=>{oe()},100),!0):(v("Contraseña incorrecta","error"),!1)}function P(){return I.isAuthenticated?Date.now()>I.sessionExpiry?(I.isAuthenticated=!1,I.sessionExpiry=null,!1):!0:!1}function Le(){I.isAuthenticated=!1,I.sessionExpiry=null,v("Sesión cerrada","info")}function oe(){if(!t.pendingAction)return;const e=t.pendingAction;switch(t.pendingAction=null,e.type){case"showAllNotes":t.showAllNotes=!0,t.allNotesPage=1;break;case"addNote":t.addNoteContactIndex=e.contactIndex,t.showAddNoteModal=!0;break;case"showContactNotes":t.selected=e.contactIndex,t.editing=null;break}m()}function De({visible:e,mode:o="login"}){return`
  ...
  `;try{const o=await Oe();e.innerHTML=`
    ...
    `}}async function je(){console.log("🚀 Inicializando aplicación...");const e=j(),o=new Promise(s=>setTimeout(()=>s("timeout"),5e3));if(await Promise.race([e,o])==="timeout"){console.log("⏰ Timeout obteniendo versión SW, usando fallback");let s=document.getElementById("sw-version-info");s&&(s.innerHTML=`
        <p class="version-text">Service Worker v0.0.91</p>
      `)}"serviceWorker"in navigator&&(navigator.serviceWorker.addEventListener("controllerchange",()=>{console.log("🔄 Service Worker actualizado, refrescando versión..."),setTimeout(()=>j(),500)}),navigator.serviceWorker.ready.then(()=>{console.log("✅ Service Worker listo, actualizando versión..."),setTimeout(()=>j(),2e3)}).catch(s=>{console.log("❌ Error esperando SW ready:",s)}),navigator.serviceWorker.addEventListener("message",s=>{s.data&&s.data.type==="SW_UPDATED"&&(console.log("📢 Service Worker actualizado a versión:",s.data.version),me(s.data.version),j())}))}let ae=0,O=!1;function Te(){let e=null;window.addEventListener("scroll",()=>{O=!0,clearTimeout(e),e=setTimeout(()=>{O=!1},100)},{passive:!0}),document.addEventListener("touchstart",()=>{ae=Date.now()},{passive:!0}),document.addEventListener("touchmove",()=>{O=!0},{passive:!0}),document.addEventListener("touchend",()=>{setTimeout(()=>{O=!1},100)},{passive:!0})}function T(e){if(e&&(e.classList.contains("add-note-contact")||e.classList.contains("edit-contact")||e.classList.contains("delete-contact")||e.classList.contains("pin-contact")||e.classList.contains("select-contact")))return!0;const o=Date.now()-ae;return!O&&o>150}document.addEventListener("DOMContentLoaded",()=>{try{console.log("=== 📱 INICIO DEBUG MÓVIL ==="),console.log("🌐 URL:",window.location.href),console.log("📱 User Agent:",navigator.userAgent),console.log("📊 Screen:",screen.width+"x"+screen.height),console.log("🖥️ Viewport:",window.innerWidth+"x"+window.innerHeight),console.log("🗂️ localStorage disponible:",!!window.localStorage),console.log("⚙️ Service Worker:","serviceWorker"in navigator),console.log("=================================");const e=document.getElementById("debug-trigger");e?(e.addEventListener("click",V),console.log("🐛 Botón de debug configurado")):console.log("❌ No se encontró el botón de debug"),console.log("🔄 Iniciando migración de contactos..."),Pe(),console.log("✅ Migración completada"),console.log("🎨 Iniciando render inicial..."),m(),console.log("✅ Render inicial completado"),console.log("⚙️ Inicializando app..."),je(),console.log("✅ App inicializada"),console.log("💾 Mostrando info de backup..."),ke(),console.log("✅ Info backup mostrada"),console.log("📱 ContactosDiarios iniciado correctamente"),console.log("🆕 Nueva funcionalidad: Contactos recientemente editados"),console.log("💡 Usa Ctrl+Shift+R para limpiar cache y forzar actualización"),console.log("🔧 También disponible: window.clearCacheAndReload()")}catch(e){console.error("💥 ERROR CRÍTICO EN INICIALIZACIÓN:",e),console.error("Stack trace:",e.stack);const o=document.querySelector("#app");o&&(o.innerHTML=`
        <div style="padding: 20px; background: #ffebee; border: 1px solid #f44336; border-radius: 8px; margin: 20px;">
      ...
      `)}});function Pe(){let e=!1;const o=Date.now();t.contacts.forEach((a,s)=>{a.lastEdited||(a.lastEdited=a.createdAt||o-(t.contacts.length-s)*1e3*60*60,e=!0),a.createdAt||(a.createdAt=a.lastEdited,e=!0)}),e&&S(t.contacts)}
