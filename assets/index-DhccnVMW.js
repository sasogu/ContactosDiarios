(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function a(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(i){if(i.ep)return;i.ep=!0;const n=a(i);fetch(i.href,n)}})();const z="0.0.92";(function(){try{const o=localStorage.getItem("app_version");if(o&&o!==z){const a=localStorage.getItem("contactos_diarios"),s=localStorage.getItem("contactos_diarios_backups"),i=localStorage.getItem("contactos_diarios_backup_fecha"),n=localStorage.getItem("contactos_diarios_webdav_config");["app_version","contactos_diarios","contactos_diarios_backups","contactos_diarios_backup_fecha","contactos_diarios_webdav_config"].forEach(u=>{u!=="contactos_diarios"&&localStorage.removeItem(u)}),"caches"in window&&caches.keys().then(u=>{u.forEach(g=>{(g.includes("contactosdiarios")||g.includes("contactos-diarios"))&&caches.delete(g)})}),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(u=>{u.forEach(g=>{g.scope.includes(window.location.origin)&&g.unregister()})}),a&&localStorage.setItem("contactos_diarios",a),s&&localStorage.setItem("contactos_diarios_backups",s),i&&localStorage.setItem("contactos_diarios_backup_fecha",i),n&&localStorage.setItem("contactos_diarios_webdav_config",n),location.reload()}localStorage.setItem("app_version",z)}catch{}})();function Q({contacts:t,filter:o,onSelect:a,onDelete:s}){let i=o?t.filter(n=>{const d=o.toLowerCase(),u=n.notes?Object.values(n.notes).join(" ").toLowerCase():"";return n.tags?.some(g=>g.toLowerCase().includes(d))||n.name?.toLowerCase().includes(d)||n.surname?.toLowerCase().includes(d)||u.includes(d)}):t;return i=i.slice().sort((n,d)=>{if(d.pinned&&!n.pinned)return 1;if(n.pinned&&!d.pinned)return-1;if(n.pinned===d.pinned){const u=n.lastEdited||0,g=d.lastEdited||0,f=Date.now(),E=24*60*60*1e3,v=f-u<E,y=f-g<E;return v&&!y?-1:y&&!v?1:v&&y?g-u:(n.surname||"").localeCompare(d.surname||"")}return 0}),`
    <div class="contact-list">
      <h2>Contactos</h2>
      <input id="tag-filter" class="tag-filter" placeholder="Buscar nombre, apellidos, etiqueta o nota..." value="${o||""}" />
      <ul>
        ${i.length===0?'<li class="empty">Sin contactos</li>':i.map((n,d)=>{const u=Date.now(),g=24*60*60*1e3,f=n.lastEdited&&u-n.lastEdited<g,E=f&&!n.pinned?" recently-edited":"";let v="";if(n.lastEdited){const y=u-n.lastEdited,N=Math.floor(y/(60*60*1e3)),B=Math.floor(y%(60*60*1e3)/(60*1e3));N<1?v=B<1?"Ahora":`${B}m`:N<24?v=`${N}h`:v=`${Math.floor(N/24)}d`}return`
          <li${n.pinned?' class="pinned"':E}>
            <div class="contact-main">
              <button class="select-contact" data-index="${t.indexOf(n)}">
                ${f&&!n.pinned?"🆕 ":""}${n.surname?n.surname+", ":""}${n.name}
                ${v&&f?`<span class="time-ago">(${v})</span>`:""}
              </button>
  ...
  `}function ne({contacts:t,visible:o,page:a=1}){let s=[];t.forEach((f,E)=>{f.notes&&Object.entries(f.notes).forEach(([v,y])=>{s.push({date:v,text:y,contact:f,contactIndex:E})})}),s.sort((f,E)=>E.date.localeCompare(f.date));const i=4,n=Math.max(1,Math.ceil(s.length/i)),d=Math.min(Math.max(a,1),n),u=(d-1)*i,g=s.slice(u,u+i);return`
...
`).filter(Boolean),[a,...s]=o;return s.map(i=>{const[n,d,u,g,f,E]=i.split(",");return{name:n?.trim()||"",surname:d?.trim()||"",phone:u?.trim()||"",email:g?.trim()||"",notes:E?JSON.parse(E):{},tags:f?f.split(";").map(v=>v.trim()):[]}})}function me(){const t=ue(e.contacts),o=new Blob([t],{type:"text/vcard"}),a=document.createElement("a");a.href=URL.createObjectURL(o),a.download="contactos.vcf",a.click()}function fe(){const t="name,surname,phone,email,tags,notes",o=e.contacts.map(n=>[n.name,n.surname,n.phone,n.email,(n.tags||[]).join(";"),JSON.stringify(n.notes||{})].map(d=>'"'+String(d).replace(/"/g,'""')+'"').join(",")),a=[t,...o].join(`
...
--- Contacto ${d+1} ---
${g}`:o.notes[u]=g})}),o.pinned=t.some(n=>n.pinned),o}function ye(t){const o=J(t);return`
  ...
  `}function xe(){const t=[];let o=0;if(e.duplicates.forEach((n,d)=>{const u=document.querySelector(`input[name="resolution-${d}"]:checked`),g=u?u.value:"skip";if(g==="merge")t.push({type:"merge",groupIndex:d,contacts:n.contacts}),o++;else if(g==="select"){const f=document.querySelector(`input[name="keep-${d}"]:checked`);if(f){const E=parseInt(f.value),v=n.contacts.filter(y=>y.originalIndex!==E).map(y=>y.originalIndex);t.push({type:"delete",groupIndex:d,toDelete:v,toKeep:E}),o++}}}),o===0){b("No hay operaciones que realizar","info");return}const a=t.filter(n=>n.type==="merge").length,s=t.filter(n=>n.type==="delete").length;let i=`¿Confirmar las siguientes operaciones?

`;if(a>0&&(i+=`🔗 Fusionar ${a} grupo(s) de contactos
`),s>0&&(i+=`🗑️ Eliminar duplicados en ${s} grupo(s)
`),i+=`
Esta acción no se puede deshacer.`,!confirm(i)){b("Operación cancelada","info");return}try{let n=0,d=0;const u=[];t.forEach(v=>{if(v.type==="merge"){const y=J(v.contacts);e.contacts.push(y),v.contacts.forEach(N=>{u.push(N.originalIndex)}),n++}else v.type==="delete"&&(v.toDelete.forEach(y=>{u.push(y)}),d+=v.toDelete.length)}),[...new Set(u)].sort((v,y)=>y-v).forEach(v=>{v<e.contacts.length&&e.contacts.splice(v,1)}),S(e.contacts);let f="Resolución completada: ";const E=[];n>0&&E.push(`${n} contacto(s) fusionado(s)`),d>0&&E.push(`${d} duplicado(s) eliminado(s)`),f+=E.join(" y "),b(f,"success"),e.showDuplicateModal=!1,e.duplicates=[],m()}catch(n){b("Error al aplicar resolución: "+n.message,"error")}}const R="contactos_diarios_auth";let I={isAuthenticated:!1,sessionExpiry:null};function G(t){let o=0;for(let a=0;a<t.length;a++){const s=t.charCodeAt(a);o=(o<<5)-o+s,o=o&o}return o.toString()}function we(t){const o=G(t);localStorage.setItem(R,o),b("Contraseña establecida correctamente","success")}function ke(t){const o=localStorage.getItem(R);return o?G(t)===o:!1}function P(){return localStorage.getItem(R)!==null}function $e(t){return ke(t)?(I.isAuthenticated=!0,I.sessionExpiry=Date.now()+30*60*1e3,b("Autenticación exitosa","success"),setTimeout(()=>{K()},100),!0):(b("Contraseña incorrecta","error"),!1)}function j(){return I.isAuthenticated?Date.now()>I.sessionExpiry?(I.isAuthenticated=!1,I.sessionExpiry=null,!1):!0:!1}function Se(){I.isAuthenticated=!1,I.sessionExpiry=null,b("Sesión cerrada","info")}function K(){if(!e.pendingAction)return;const t=e.pendingAction;switch(e.pendingAction=null,t.type){case"showAllNotes":e.showAllNotes=!0,e.allNotesPage=1;break;case"addNote":e.addNoteContactIndex=t.contactIndex,e.showAddNoteModal=!0;break;case"showContactNotes":e.selected=t.contactIndex,e.editing=null;break}m()}function Ie({visible:t,mode:o="login"}){return`
  ...
  `;try{const o=await Ne();t.innerHTML=`
    ...
    `}}async function Ce(){console.log("🚀 Inicializando aplicación...");const t=M(),o=new Promise(s=>setTimeout(()=>s("timeout"),5e3));if(await Promise.race([t,o])==="timeout"){console.log("⏰ Timeout obteniendo versión SW, usando fallback");let s=document.getElementById("sw-version-info");s&&(s.innerHTML=`
      ...
      `)}"serviceWorker"in navigator&&(navigator.serviceWorker.addEventListener("controllerchange",()=>{console.log("🔄 Service Worker actualizado, refrescando versión..."),setTimeout(()=>M(),500)}),navigator.serviceWorker.ready.then(()=>{console.log("✅ Service Worker listo, actualizando versión..."),setTimeout(()=>M(),2e3)}).catch(s=>{console.log("❌ Error esperando SW ready:",s)}),navigator.serviceWorker.addEventListener("message",s=>{s.data&&s.data.type==="SW_UPDATED"&&(console.log("📢 Service Worker actualizado a versión:",s.data.version),ie(s.data.version),M())}))}let Y=0,D=!1;function Ae(){let t=null;window.addEventListener("scroll",()=>{D=!0,clearTimeout(t),t=setTimeout(()=>{D=!1},100)},{passive:!0}),document.addEventListener("touchstart",()=>{Y=Date.now()},{passive:!0}),document.addEventListener("touchmove",()=>{D=!0},{passive:!0}),document.addEventListener("touchend",()=>{setTimeout(()=>{D=!1},100)},{passive:!0})}function L(){const t=Date.now()-Y;return!D&&t>150}document.addEventListener("DOMContentLoaded",()=>{Z(),m(),Ce(),he(),console.log("📱 ContactosDiarios iniciado correctamente"),console.log("🆕 Nueva funcionalidad: Contactos recientemente editados"),console.log("💡 Usa Ctrl+Shift+R para limpiar cache y forzar actualización"),console.log("🔧 También disponible: window.clearCacheAndReload()")});function Z(){let t=!1;const o=Date.now();e.contacts.forEach((a,s)=>{a.lastEdited||(a.lastEdited=a.createdAt||o-s*1e3*60,t=!0),a.createdAt||(a.createdAt=a.lastEdited,t=!0)}),t&&(S(e.contacts),console.log("📅 Contactos migrados con fechas de edición"))}document.addEventListener("DOMContentLoaded",()=>{Z()});
