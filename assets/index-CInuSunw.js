(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const l of document.querySelectorAll('link[rel="modulepreload"]'))s(l);new MutationObserver(l=>{for(const n of l)if(n.type==="childList")for(const a of n.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&s(a)}).observe(document,{childList:!0,subtree:!0});function c(l){const n={};return l.integrity&&(n.integrity=l.integrity),l.referrerPolicy&&(n.referrerPolicy=l.referrerPolicy),l.crossOrigin==="use-credentials"?n.credentials="include":l.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(l){if(l.ep)return;l.ep=!0;const n=c(l);fetch(l.href,n)}})();const f="0.0.18";(function(){try{const e=localStorage.getItem("app_version");if(e&&e!==f){const c=localStorage.getItem("contactos_diarios");Object.keys(localStorage).forEach(s=>{s!=="contactos_diarios"&&localStorage.removeItem(s)}),"caches"in window&&caches.keys().then(s=>s.forEach(l=>caches.delete(l))),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(s=>s.forEach(l=>l.unregister())),c&&localStorage.setItem("contactos_diarios",c),location.reload()}localStorage.setItem("app_version",f)}catch{}})();function h({contacts:o,filter:e,onSelect:c,onDelete:s}){let l=e?o.filter(n=>{const a=e.toLowerCase(),i=n.notes?Object.values(n.notes).join(" ").toLowerCase():"";return n.tags?.some(r=>r.toLowerCase().includes(a))||n.name?.toLowerCase().includes(a)||n.surname?.toLowerCase().includes(a)||i.includes(a)}):o;return l=l.slice().sort((n,a)=>a.pinned&&!n.pinned?1:n.pinned&&!a.pinned?-1:(n.surname||"").localeCompare(a.surname||"")),`
    <div class="contact-list">
      ...
      <ul>
        ${l.length===0?'<li class="empty">Sin contactos</li>':l.map((n,a)=>`
  ...
  `}function x({contacts:o,visible:e}){let c=[];return o.forEach((s,l)=>{s.notes&&Object.entries(s.notes).forEach(([n,a])=>{c.push({date:n,text:a,contact:s,contactIndex:l})})}),c.sort((s,l)=>l.date.localeCompare(s.date)),`
    <div id="all-notes-modal" class="modal" style="display:${e?"flex":"none"}">
      <div class="modal-content" style="max-width: 500px;">
        <h3>Notas de todos los contactos</h3>
        <ul class="note-history">
          ${c.length===0?"<li>No hay notas registradas.</li>":c.map(s=>`
            <li>
              <b>${s.date}</b> — <span style="color:#3a4a7c">${s.contact.surname?s.contact.surname+", ":""}${s.contact.name}</span><br/>
              <span>${s.text}</span>
              <a href="#" class="edit-note-link" data-contact="${s.contactIndex}" data-date="${s.date}" style="margin-left:0.5em;color:#646cff;font-size:0.95em;">Editar nota</a>
            </li>
          `).join("")}
        </ul>
        <div class="form-actions">
        ...
        <div class="backup-btns" style="display:flex;flex-wrap:wrap;gap:0.5rem;justify-content:center;">
          ${e.length===0?"<span>Sin copias locales.</span>":e.map(c=>`
            <div class="backup-btn-group" style="display:flex;align-items:center;gap:0.3rem;">
              ...
              <button class="share-backup-btn" data-fecha="${c.fecha}" title="Compartir backup" style="background:#06b6d4;padding:0.4rem 0.7rem;font-size:1.1em;">4e4</button>
            </div>
          `).join("")}
        </div>
