(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const W="0.0.81";(function(){try{const t=localStorage.getItem("app_version");if(t&&t!==W){const s=localStorage.getItem("contactos_diarios"),i=localStorage.getItem("contactos_diarios_backups"),a=localStorage.getItem("contactos_diarios_backup_fecha"),o=localStorage.getItem("contactos_diarios_webdav_config");["app_version","contactos_diarios","contactos_diarios_backups","contactos_diarios_backup_fecha","contactos_diarios_webdav_config"].forEach(m=>{m!=="contactos_diarios"&&localStorage.removeItem(m)}),"caches"in window&&caches.keys().then(m=>{m.forEach(b=>{(b.includes("contactosdiarios")||b.includes("contactos-diarios"))&&caches.delete(b)})}),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(m=>{m.forEach(b=>{b.scope.includes(window.location.origin)&&b.unregister()})}),s&&localStorage.setItem("contactos_diarios",s),i&&localStorage.setItem("contactos_diarios_backups",i),a&&localStorage.setItem("contactos_diarios_backup_fecha",a),o&&localStorage.setItem("contactos_diarios_webdav_config",o),location.reload()}localStorage.setItem("app_version",W)}catch{}})();function ee({contacts:n,filter:t,onSelect:s,onDelete:i}){let a=t?n.filter(o=>{const d=t.toLowerCase(),m=o.notes?Object.values(o.notes).join(" ").toLowerCase():"";return o.tags?.some(b=>b.toLowerCase().includes(d))||o.name?.toLowerCase().includes(d)||o.surname?.toLowerCase().includes(d)||m.includes(d)}):n;return a=a.slice().sort((o,d)=>d.pinned&&!o.pinned?1:o.pinned&&!d.pinned?-1:(o.surname||"").localeCompare(d.surname||"")),`
  ...
  `}function ae({contacts:n,visible:t,page:s=1}){let i=[];n.forEach((h,x)=>{h.notes&&Object.entries(h.notes).forEach(([v,w])=>{i.push({date:v,text:w,contact:h,contactIndex:x})})}),i.sort((h,x)=>x.date.localeCompare(h.date));const a=4,o=Math.max(1,Math.ceil(i.length/a)),d=Math.min(Math.max(s,1),o),m=(d-1)*a,b=i.slice(m,m+a);return`
  ...
  `;const a=t==="success"?"✅":t==="error"?"❌":t==="warning"?"⚠️":"ℹ️";i.innerHTML=`${a} ${n}`,s.appendChild(i),setTimeout(()=>{i.style.opacity="1",i.style.transform="translateX(0)"},10);const o=()=>{i.style.opacity="0",i.style.transform="translateX(100%)",setTimeout(()=>{i.parentNode&&i.parentNode.removeChild(i)},300)};i.onclick=o,setTimeout(o,4e3)}function O(n){const t=[];return!n.name||n.name.trim().length===0?t.push("El nombre es obligatorio"):n.name.trim().length<2?t.push("El nombre debe tener al menos 2 caracteres"):n.name.trim().length>50&&t.push("El nombre no puede tener más de 50 caracteres"),n.surname&&n.surname.trim().length>50&&t.push("Los apellidos no pueden tener más de 50 caracteres"),n.phone&&n.phone.trim().length>0&&(/^[\d\s\-\+\(\)\.]{6,20}$/.test(n.phone.trim())||t.push("El teléfono debe contener solo números, espacios y caracteres válidos (6-20 caracteres)")),n.email&&n.email.trim().length>0&&(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n.email.trim())?n.email.trim().length>100&&t.push("El email no puede tener más de 100 caracteres"):t.push("El email debe tener un formato válido")),t}function _(n){const t=[];return!n||n.trim().length===0?(t.push("La nota no puede estar vacía"),t):(n.trim().length>1e3&&t.push("La nota no puede tener más de 1000 caracteres"),n.trim().length<3&&t.push("La nota debe tener al menos 3 caracteres"),/^[\w\s\.\,\;\:\!\?\-\(\)\[\]\"\'\/\@\#\$\%\&\*\+\=\<\>\{\}\|\~\`\ñÑáéíóúÁÉÍÓÚüÜ]*$/.test(n)||t.push("La nota contiene caracteres no permitidos"),t)}const G="contactos_diarios";let e={contacts:ce(),selected:null,notes:"",editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,showAddNoteModal:!1,addNoteContactIndex:null,allNotesPage:1,duplicates:[],showDuplicateModal:!1,showAuthModal:!1,authMode:"login",pendingAction:null};function ce(){try{return JSON.parse(localStorage.getItem(G))||[]}catch{return[]}}function I(n){localStorage.setItem(G,JSON.stringify(n))}function p(){const n=document.querySelector("#app"),t=e.editing!==null?e.contacts[e.editing]:null,s=e.selected!==null?e.contacts[e.selected].notes||{}:{};n.innerHTML=`
    ...
    ${ve({duplicates:e.duplicates,visible:e.showDuplicateModal})} <!-- Modal de gestión de duplicados -->
...
${b}`:t.notes[m]=b})}),t.pinned=n.some(o=>o.pinned),t}function be(n){const t=X(n);return`
  ...
  `}function ve({duplicates:n,visible:t}){return!t||n.length===0?'<div id="duplicate-modal" class="modal" style="display:none"></div>':`
...
Esta acción no se puede deshacer.`,!confirm(a)){g("Operación cancelada","info");return}try{let o=0,d=0;const m=[];n.forEach(v=>{if(v.type==="merge"){const w=X(v.contacts);e.contacts.push(w),v.contacts.forEach(M=>{m.push(M.originalIndex)}),o++}else v.type==="delete"&&(v.toDelete.forEach(w=>{m.push(w)}),d+=v.toDelete.length)}),[...new Set(m)].sort((v,w)=>w-v).forEach(v=>{v<e.contacts.length&&e.contacts.splice(v,1)}),I(e.contacts);let h="Resolución completada: ";const x=[];o>0&&x.push(`${o} contacto(s) fusionado(s)`),d>0&&x.push(`${d} duplicado(s) eliminado(s)`),h+=x.join(" y "),g(h,"success"),e.showDuplicateModal=!1,e.duplicates=[],p()}catch(o){g("Error al aplicar resolución: "+o.message,"error")}}const T="contactos_diarios_auth";let S={isAuthenticated:!1,sessionExpiry:null};function Y(n){let t=0;for(let s=0;s<n.length;s++){const i=n.charCodeAt(s);t=(t<<5)-t+i,t=t&t}return t.toString()}function xe(n){const t=Y(n);localStorage.setItem(T,t),g("Contraseña establecida correctamente","success")}function Ee(n){const t=localStorage.getItem(T);return t?Y(n)===t:!1}function q(){return localStorage.getItem(T)!==null}function we(n){return Ee(n)?(S.isAuthenticated=!0,S.sessionExpiry=Date.now()+30*60*1e3,g("Autenticación exitosa","success"),setTimeout(()=>{Z()},100),!0):(g("Contraseña incorrecta","error"),!1)}function L(){return S.isAuthenticated?Date.now()>S.sessionExpiry?(S.isAuthenticated=!1,S.sessionExpiry=null,!1):!0:!1}function $e(){S.isAuthenticated=!1,S.sessionExpiry=null,g("Sesión cerrada","info")}function Z(){if(!e.pendingAction)return;const n=e.pendingAction;switch(e.pendingAction=null,n.type){case"showAllNotes":e.showAllNotes=!0,e.allNotesPage=1;break;case"addNote":e.addNoteContactIndex=n.contactIndex,e.showAddNoteModal=!0;break;case"showContactNotes":e.selected=n.contactIndex,e.editing=null;break}p()}function ke({visible:n,mode:t="login"}){return`
  ...
  `}function Ne(){return window.innerWidth<=700||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}function Ie(){if(/iPad|iPhone|iPod/.test(navigator.userAgent)&&(document.body.style.webkitOverflowScrolling="touch"),window.matchMedia("(display-mode: standalone)").matches&&(document.body.classList.add("pwa-mode"),/iPhone/.test(navigator.userAgent)&&(document.body.style.paddingTop="env(safe-area-inset-top)")),Ne()){const n=document.querySelector('meta[name="viewport"]');n&&n.setAttribute("content","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover")}}document.addEventListener("DOMContentLoaded",()=>{p(),Ie()});let R=!1,P=null;function J(){R=!0,P&&clearTimeout(P),P=setTimeout(()=>{R=!1},150)}function D(){return!R}function Q(){const n=document.querySelector(".contact-list ul");n&&(n.addEventListener("scroll",J,{passive:!0}),n.addEventListener("touchmove",J,{passive:!0}))}Q();let B=null;const k=document.createElement("button");k.textContent="📲 Instalar en tu dispositivo";k.className="add-btn";k.style.display="none";k.style.position="fixed";k.style.bottom="1.5rem";k.style.left="50%";k.style.transform="translateX(-50%)";k.style.zIndex="3000";document.body.appendChild(k);window.addEventListener("beforeinstallprompt",n=>{n.preventDefault(),B=n,k.style.display="block"});k.addEventListener("click",async()=>{if(B){B.prompt();const{outcome:n}=await B.userChoice;n==="accepted"&&(k.style.display="none"),B=null}});window.addEventListener("appinstalled",()=>{k.style.display="none"});async function Se(){try{if("serviceWorker"in navigator&&navigator.serviceWorker.controller){const n=await navigator.serviceWorker.getRegistration();if(n&&n.active){const i=(await(await fetch(n.active.scriptURL)).text()).match(/CACHE_VERSION\s*=\s*['"`]([^'"`]+)['"`]/);if(i)return i[1]}}return"No disponible"}catch(n){return console.log("Error obteniendo versión SW:",n),"Error"}}async function Ae(){const n=await Se();let t=document.getElementById("sw-version-info");t||(t=document.createElement("div"),t.id="sw-version-info",t.className="version-info",document.body.appendChild(t)),t.innerHTML=`
