(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const U="0.0.82";(function(){try{const t=localStorage.getItem("app_version");if(t&&t!==U){const s=localStorage.getItem("contactos_diarios"),i=localStorage.getItem("contactos_diarios_backups"),a=localStorage.getItem("contactos_diarios_backup_fecha"),o=localStorage.getItem("contactos_diarios_webdav_config");["app_version","contactos_diarios","contactos_diarios_backups","contactos_diarios_backup_fecha","contactos_diarios_webdav_config"].forEach(m=>{m!=="contactos_diarios"&&localStorage.removeItem(m)}),"caches"in window&&caches.keys().then(m=>{m.forEach(b=>{(b.includes("contactosdiarios")||b.includes("contactos-diarios"))&&caches.delete(b)})}),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(m=>{m.forEach(b=>{b.scope.includes(window.location.origin)&&b.unregister()})}),s&&localStorage.setItem("contactos_diarios",s),i&&localStorage.setItem("contactos_diarios_backups",i),a&&localStorage.setItem("contactos_diarios_backup_fecha",a),o&&localStorage.setItem("contactos_diarios_webdav_config",o),location.reload()}localStorage.setItem("app_version",U)}catch{}})();function te({contacts:n,filter:t,onSelect:s,onDelete:i}){let a=t?n.filter(o=>{const d=t.toLowerCase(),m=o.notes?Object.values(o.notes).join(" ").toLowerCase():"";return o.tags?.some(b=>b.toLowerCase().includes(d))||o.name?.toLowerCase().includes(d)||o.surname?.toLowerCase().includes(d)||m.includes(d)}):n;return a=a.slice().sort((o,d)=>d.pinned&&!o.pinned?1:o.pinned&&!d.pinned?-1:(o.surname||"").localeCompare(d.surname||"")),`
  ...
  `}function oe({notes:n}){if(!L())return`
  ...
  `}function se({contacts:n,visible:t,page:s=1}){let i=[];n.forEach((h,x)=>{h.notes&&Object.entries(h.notes).forEach(([v,$])=>{i.push({date:v,text:$,contact:h,contactIndex:x})})}),i.sort((h,x)=>x.date.localeCompare(h.date));const a=4,o=Math.max(1,Math.ceil(i.length/a)),d=Math.min(Math.max(s,1),o),m=(d-1)*a,b=i.slice(m,m+a);return`
  ...
  `}function ie({visible:n,backups:t}){return`
  ...
  `}function ce({visible:n,contactIndex:t}){const s=t!==null?e.contacts[t]:null,i=new Date,o=new Date(i.toLocaleString("en-US",{timeZone:"Europe/Madrid"})).toISOString().slice(0,10);return`
  ...
  `;const a=t==="success"?"✅":t==="error"?"❌":t==="warning"?"⚠️":"ℹ️";i.innerHTML=`${a} ${n}`,s.appendChild(i),setTimeout(()=>{i.style.opacity="1",i.style.transform="translateX(0)"},10);const o=()=>{i.style.opacity="0",i.style.transform="translateX(100%)",setTimeout(()=>{i.parentNode&&i.parentNode.removeChild(i)},300)};i.onclick=o,setTimeout(o,4e3)}function O(n){const t=[];return!n.name||n.name.trim().length===0?t.push("El nombre es obligatorio"):n.name.trim().length<2?t.push("El nombre debe tener al menos 2 caracteres"):n.name.trim().length>50&&t.push("El nombre no puede tener más de 50 caracteres"),n.surname&&n.surname.trim().length>50&&t.push("Los apellidos no pueden tener más de 50 caracteres"),n.phone&&n.phone.trim().length>0&&(/^[\d\s\-\+\(\)\.]{6,20}$/.test(n.phone.trim())||t.push("El teléfono debe contener solo números, espacios y caracteres válidos (6-20 caracteres)")),n.email&&n.email.trim().length>0&&(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n.email.trim())?n.email.trim().length>100&&t.push("El email no puede tener más de 100 caracteres"):t.push("El email debe tener un formato válido")),t}function _(n){const t=[];return!n||n.trim().length===0?(t.push("La nota no puede estar vacía"),t):(n.trim().length>1e3&&t.push("La nota no puede tener más de 1000 caracteres"),n.trim().length<3&&t.push("La nota debe tener al menos 3 caracteres"),/^[\w\s\.\,\;\:\!\?\-\(\)\[\]\"\'\/\@\#\$\%\&\*\+\=\<\>\{\}\|\~\`\ñÑáéíóúÁÉÍÓÚüÜ]*$/.test(n)||t.push("La nota contiene caracteres no permitidos"),t)}const H="contactos_diarios";let e={contacts:le(),selected:null,notes:"",editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,showAddNoteModal:!1,addNoteContactIndex:null,allNotesPage:1,duplicates:[],showDuplicateModal:!1,showAuthModal:!1,authMode:"login",pendingAction:null};function le(){try{return JSON.parse(localStorage.getItem(H))||[]}catch{return[]}}function N(n){localStorage.setItem(H,JSON.stringify(n))}function p(){const n=document.querySelector("#app"),t=e.editing!==null?e.contacts[e.editing]:null,s=e.selected!==null?e.contacts[e.selected].notes||{}:{};n.innerHTML=`
    ...
    <div class="main-grid">
      ...
      <div>
        ${e.editing!==null?ne({contact:t}):""}
        ${e.selected!==null&&e.editing===null?oe({notes:s}):""}
      </div>
    </div>
...
¿Deseas ver más detalles en la consola del navegador?`)&&console.table(l)}});const v=document.getElementById("cancel-duplicate-resolution");v&&(v.onclick=()=>{e.showDuplicateModal=!1,e.duplicates=[],p()});const $=document.getElementById("apply-resolution");$&&($.onclick=xe),document.querySelectorAll('input[name^="resolution-"]').forEach(l=>{l.addEventListener("change",()=>{const c=l.name.split("-")[1],r=document.getElementById(`merge-section-${c}`),u=document.getElementById(`individual-section-${c}`);l.value==="merge"?(r.style.display="block",u.style.display="none"):l.value==="select"?(r.style.display="none",u.style.display="block"):(r.style.display="none",u.style.display="none")})}),document.querySelectorAll('.resolution-option input[type="radio"]').forEach(l=>{l.addEventListener("change",()=>{const c=l.name;document.querySelectorAll(`input[name="${c}"]`).forEach(r=>{r.closest(".resolution-option").classList.remove("selected")}),l.closest(".resolution-option").classList.add("selected")})});const M=document.getElementById("unlock-notes-btn");M&&(M.onclick=()=>{e.selected!==null&&(e.pendingAction={type:"showContactNotes",contactIndex:e.selected}),q()?e.authMode="login":e.authMode="setup",e.showAuthModal=!0,p()});const V=document.getElementById("logout-btn");V&&(V.onclick=()=>{ke(),p()});const C=document.getElementById("auth-form");C&&!C.hasAttribute("data-handler-added")&&(C.setAttribute("data-handler-added","true"),C.onsubmit=l=>{l.preventDefault();const c=document.getElementById("auth-password").value.trim();if(!c){g("Por favor, introduce una contraseña","warning");return}if(e.authMode==="setup"){const r=document.getElementById("auth-password-confirm").value.trim();if(c!==r){g("Las contraseñas no coinciden","error");return}if(c.length<4){g("La contraseña debe tener al menos 4 caracteres","warning");return}we(c),S.isAuthenticated=!0,S.sessionExpiry=Date.now()+30*60*1e3,e.showAuthModal=!1,C.reset(),setTimeout(()=>{Q()},100),p()}else $e(c)?(e.showAuthModal=!1,C.reset(),p()):(document.getElementById("auth-password").value="",document.getElementById("auth-password").focus())});const z=document.getElementById("cancel-auth");z&&(z.onclick=()=>{e.showAuthModal=!1,e.pendingAction=null,p()});const j=document.getElementById("auth-modal");if(j){j.onclick=r=>{r.target===j&&(e.showAuthModal=!1,e.pendingAction=null,p())};const l=document.getElementById("auth-password");if(l){const r=window.innerWidth<=700?300:100;setTimeout(()=>{l.focus(),window.innerWidth<=700&&l.scrollIntoView({behavior:"smooth",block:"center"})},r)}const c=r=>{r.key==="Escape"&&e.showAuthModal&&(e.showAuthModal=!1,e.pendingAction=null,p())};document.addEventListener("keydown",c)}}function de(n){const t=[],s=n.split("END:VCARD");for(const i of s){const a=/FN:([^\n]*)/.exec(i)?.[1]?.trim(),o=/N:.*;([^;\n]*)/.exec(i)?.[1]?.trim()||"",d=/TEL.*:(.+)/.exec(i)?.[1]?.trim(),m=/EMAIL.*:(.+)/.exec(i)?.[1]?.trim();a&&t.push({name:a,surname:o,phone:d||"",email:m||"",notes:{},tags:[]})}return t}function ue(n){return n.map(t=>`BEGIN:VCARD
VERSION:3.0
FN:${t.name}
N:${t.surname||""};;;;
TEL:${t.phone||""}
EMAIL:${t.email||""}
END:VCARD`).join(`
`)}function pe(n){const t=n.split(`
`).filter(Boolean),[s,...i]=t;return i.map(a=>{const[o,d,m,b,h,x]=a.split(",");return{name:o?.trim()||"",surname:d?.trim()||"",phone:m?.trim()||"",email:b?.trim()||"",notes:x?JSON.parse(x):{},tags:h?h.split(";").map(v=>v.trim()):[]}})}function me(){const n=ue(e.contacts),t=new Blob([n],{type:"text/vcard"}),s=document.createElement("a");s.href=URL.createObjectURL(t),s.download="contactos.vcf",s.click()}function fe(){const n="name,surname,phone,email,tags,notes",t=e.contacts.map(o=>[o.name,o.surname,o.phone,o.email,(o.tags||[]).join(";"),JSON.stringify(o.notes||{})].map(d=>'"'+String(d).replace(/"/g,'""')+'"').join(",")),s=[n,...t].join(`
`),i=new Blob([s],{type:"text/csv"}),a=document.createElement("a");a.href=URL.createObjectURL(i),a.download="contactos.csv",a.click()}function ge(){const n=new Blob([JSON.stringify(e.contacts,null,2)],{type:"application/json"}),t=document.createElement("a");t.href=URL.createObjectURL(n),t.download="contactos.json",t.click()}function K(){const n=new Date,s=new Date(n.toLocaleString("en-US",{timeZone:"Europe/Madrid"})).toISOString().slice(0,10);let i=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]");i.find(a=>a.fecha===s)||(i.push({fecha:s,datos:e.contacts}),i.length>10&&(i=i.slice(-10)),localStorage.setItem("contactos_diarios_backups",JSON.stringify(i))),localStorage.setItem("contactos_diarios_backup_fecha",s)}setInterval(K,60*60*1e3);K();function he(n){if(!confirm("¿Seguro que quieres restaurar la copia de seguridad del "+n+"? Se sobrescribirán los contactos actuales."))return;const s=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]").find(i=>i.fecha===n);s?(e.contacts=s.datos,N(e.contacts),p(),alert("Backup restaurado correctamente.")):alert("No se encontró la copia seleccionada.")}function X(n,t){const s=i=>i?i.toLowerCase().replace(/\s+/g," ").trim():"";return!!(s(n.name)===s(t.name)&&s(n.surname)===s(t.surname)||n.phone&&t.phone&&n.phone.replace(/\s+/g,"")===t.phone.replace(/\s+/g,"")||n.email&&t.email&&s(n.email)===s(t.email))}function be(){const n=[],t=new Set;for(let s=0;s<e.contacts.length;s++){if(t.has(s))continue;const i=[{...e.contacts[s],originalIndex:s}];t.add(s);for(let a=s+1;a<e.contacts.length;a++)t.has(a)||X(e.contacts[s],e.contacts[a])&&(i.push({...e.contacts[a],originalIndex:a}),t.add(a));i.length>1&&n.push({contacts:i})}return n}function Y(n){if(n.length===0)return null;if(n.length===1)return n[0];const t={name:"",surname:"",phone:"",email:"",tags:[],notes:{},pinned:!1};let s="",i="";n.forEach(o=>{o.name&&o.name.length>s.length&&(s=o.name),o.surname&&o.surname.length>i.length&&(i=o.surname)}),t.name=s,t.surname=i,t.phone=n.find(o=>o.phone)?.phone||"",t.email=n.find(o=>o.email)?.email||"";const a=new Set;return n.forEach(o=>{o.tags&&o.tags.forEach(d=>a.add(d))}),t.tags=Array.from(a),n.forEach((o,d)=>{o.notes&&Object.entries(o.notes).forEach(([m,b])=>{t.notes[m]?t.notes[m]+=`
--- Contacto ${d+1} ---
${b}`:t.notes[m]=b})}),t.pinned=n.some(o=>o.pinned),t}function ve(n){const t=Y(n);return`
  ...
  `}function xe(){const n=[];let t=0;if(e.duplicates.forEach((o,d)=>{const m=document.querySelector(`input[name="resolution-${d}"]:checked`),b=m?m.value:"skip";if(b==="merge")n.push({type:"merge",groupIndex:d,contacts:o.contacts}),t++;else if(b==="select"){const h=document.querySelector(`input[name="keep-${d}"]:checked`);if(h){const x=parseInt(h.value),v=o.contacts.filter($=>$.originalIndex!==x).map($=>$.originalIndex);n.push({type:"delete",groupIndex:d,toDelete:v,toKeep:x}),t++}}}),t===0){g("No hay operaciones que realizar","info");return}const s=n.filter(o=>o.type==="merge").length,i=n.filter(o=>o.type==="delete").length;let a=`¿Confirmar las siguientes operaciones?

`;if(s>0&&(a+=`🔗 Fusionar ${s} grupo(s) de contactos
`),i>0&&(a+=`🗑️ Eliminar duplicados en ${i} grupo(s)
`),a+=`
Esta acción no se puede deshacer.`,!confirm(a)){g("Operación cancelada","info");return}try{let o=0,d=0;const m=[];n.forEach(v=>{if(v.type==="merge"){const $=Y(v.contacts);e.contacts.push($),v.contacts.forEach(M=>{m.push(M.originalIndex)}),o++}else v.type==="delete"&&(v.toDelete.forEach($=>{m.push($)}),d+=v.toDelete.length)}),[...new Set(m)].sort((v,$)=>$-v).forEach(v=>{v<e.contacts.length&&e.contacts.splice(v,1)}),N(e.contacts);let h="Resolución completada: ";const x=[];o>0&&x.push(`${o} contacto(s) fusionado(s)`),d>0&&x.push(`${d} duplicado(s) eliminado(s)`),h+=x.join(" y "),g(h,"success"),e.showDuplicateModal=!1,e.duplicates=[],p()}catch(o){g("Error al aplicar resolución: "+o.message,"error")}}const T="contactos_diarios_auth";let S={isAuthenticated:!1,sessionExpiry:null};function Z(n){let t=0;for(let s=0;s<n.length;s++){const i=n.charCodeAt(s);t=(t<<5)-t+i,t=t&t}return t.toString()}function we(n){const t=Z(n);localStorage.setItem(T,t),g("Contraseña establecida correctamente","success")}function Ee(n){const t=localStorage.getItem(T);return t?Z(n)===t:!1}function q(){return localStorage.getItem(T)!==null}function $e(n){return Ee(n)?(S.isAuthenticated=!0,S.sessionExpiry=Date.now()+30*60*1e3,g("Autenticación exitosa","success"),setTimeout(()=>{Q()},100),!0):(g("Contraseña incorrecta","error"),!1)}function L(){return S.isAuthenticated?Date.now()>S.sessionExpiry?(S.isAuthenticated=!1,S.sessionExpiry=null,!1):!0:!1}function ke(){S.isAuthenticated=!1,S.sessionExpiry=null,g("Sesión cerrada","info")}function Q(){if(!e.pendingAction)return;const n=e.pendingAction;switch(e.pendingAction=null,n.type){case"showAllNotes":e.showAllNotes=!0,e.allNotesPage=1;break;case"addNote":e.addNoteContactIndex=n.contactIndex,e.showAddNoteModal=!0;break;case"showContactNotes":e.selected=n.contactIndex,e.editing=null;break}p()}function Ie({visible:n,mode:t="login"}){return`
  ...
  `}function Ne(){return window.innerWidth<=700||/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}function Se(){if(/iPad|iPhone|iPod/.test(navigator.userAgent)&&(document.body.style.webkitOverflowScrolling="touch"),window.matchMedia("(display-mode: standalone)").matches&&(document.body.classList.add("pwa-mode"),/iPhone/.test(navigator.userAgent)&&(document.body.style.paddingTop="env(safe-area-inset-top)")),Ne()){const n=document.querySelector('meta[name="viewport"]');n&&n.setAttribute("content","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover")}}document.addEventListener("DOMContentLoaded",()=>{p(),Se()});let R=!1,P=null;function G(){R=!0,P&&clearTimeout(P),P=setTimeout(()=>{R=!1},150)}function D(){return!R}function ee(){const n=document.querySelector(".contact-list ul");n&&(n.addEventListener("scroll",G,{passive:!0}),n.addEventListener("touchmove",G,{passive:!0}))}ee();let B=null;const E=document.createElement("button");E.textContent="📲 Instalar en tu dispositivo";E.className="add-btn";E.style.display="none";E.style.position="fixed";E.style.bottom="1.5rem";E.style.left="50%";E.style.transform="translateX(-50%)";E.style.zIndex="3000";document.body.appendChild(E);function F(){return!!(window.matchMedia("(display-mode: standalone)").matches||window.matchMedia("(display-mode: fullscreen)").matches||window.matchMedia("(display-mode: minimal-ui)").matches||window.navigator.standalone===!0)}F()&&(E.style.display="none");window.addEventListener("beforeinstallprompt",n=>{n.preventDefault(),B=n,F()||(E.style.display="block")});E.addEventListener("click",async()=>{if(B){B.prompt();const{outcome:n}=await B.userChoice;n==="accepted"&&(E.style.display="none"),B=null}});window.addEventListener("appinstalled",()=>{E.style.display="none"});setInterval(()=>{F()&&E.style.display!=="none"&&(E.style.display="none")},2e3);async function Ae(){try{if("serviceWorker"in navigator&&navigator.serviceWorker.controller){const n=await navigator.serviceWorker.getRegistration();if(n&&n.active){const i=(await(await fetch(n.active.scriptURL)).text()).match(/CACHE_VERSION\s*=\s*['"`]([^'"`]+)['"`]/);if(i)return i[1]}}return"No disponible"}catch(n){return console.log("Error obteniendo versión SW:",n),"Error"}}async function Ce(){const n=await Ae();let t=document.getElementById("sw-version-info");t||(t=document.createElement("div"),t.id="sw-version-info",t.className="version-info",document.body.appendChild(t)),t.innerHTML=`
    <p class="version-text">Service Worker v${n}</p>
