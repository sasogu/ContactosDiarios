(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))l(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const a of n.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&l(a)}).observe(document,{childList:!0,subtree:!0});function c(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function l(s){if(s.ep)return;s.ep=!0;const n=c(s);fetch(s.href,n)}})();const y="0.0.23";(function(){try{const e=localStorage.getItem("app_version");if(e&&e!==y){const c=localStorage.getItem("contactos_diarios");Object.keys(localStorage).forEach(l=>{l!=="contactos_diarios"&&localStorage.removeItem(l)}),"caches"in window&&caches.keys().then(l=>l.forEach(s=>caches.delete(s))),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(l=>l.forEach(s=>s.unregister())),c&&localStorage.setItem("contactos_diarios",c),location.reload()}localStorage.setItem("app_version",y)}catch{}})();function E({contacts:o,filter:e,onSelect:c,onDelete:l}){let s=e?o.filter(n=>{const a=e.toLowerCase(),i=n.notes?Object.values(n.notes).join(" ").toLowerCase():"";return n.tags?.some(r=>r.toLowerCase().includes(a))||n.name?.toLowerCase().includes(a)||n.surname?.toLowerCase().includes(a)||i.includes(a)}):o;return s=s.slice().sort((n,a)=>a.pinned&&!n.pinned?1:n.pinned&&!a.pinned?-1:(n.surname||"").localeCompare(a.surname||"")),`
  ...
  `}function $({contacts:o,visible:e}){let c=[];return o.forEach((l,s)=>{l.notes&&Object.entries(l.notes).forEach(([n,a])=>{c.push({date:n,text:a,contact:l,contactIndex:s})})}),c.sort((l,s)=>s.date.localeCompare(l.date)),`
  ...
  `}function S({visible:o,backups:e}){return`
  ...
  `}const h="contactos_diarios";let t={contacts:B(),selected:null,editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1};function B(){try{return JSON.parse(localStorage.getItem(h))||[]}catch{return[]}}function p(o){localStorage.setItem(h,JSON.stringify(o))}function d(){const o=document.querySelector("#app"),e=t.editing!==null?t.contacts[t.editing]:null,c=t.selected!==null?t.contacts[t.selected].notes||{}:{};o.innerHTML=`
...
EMAIL:${e.email||""}
END:VCARD`).join(`
`)}function O(o){const e=o.split(`
`).filter(Boolean),[c,...l]=e;return l.map(s=>{const[n,a,i,r,u,m]=s.split(",");return{name:n?.trim()||"",surname:a?.trim()||"",phone:i?.trim()||"",email:r?.trim()||"",notes:m?JSON.parse(m):{},tags:u?u.split(";").map(b=>b.trim()):[]}})}function j(){const o=C(t.contacts),e=new Blob([o],{type:"text/vcard"}),c=document.createElement("a");c.href=URL.createObjectURL(e),c.download="contactos.vcf",c.click()}function L(){const o="name,surname,phone,email,tags,notes",e=t.contacts.map(n=>[n.name,n.surname,n.phone,n.email,(n.tags||[]).join(";"),JSON.stringify(n.notes||{})].map(a=>'"'+String(a).replace(/"/g,'""')+'"').join(",")),c=[o,...e].join(`
`),l=new Blob([c],{type:"text/csv"}),s=document.createElement("a");s.href=URL.createObjectURL(l),s.download="contactos.csv",s.click()}function A(){const o=new Blob([JSON.stringify(t.contacts,null,2)],{type:"application/json"}),e=document.createElement("a");e.href=URL.createObjectURL(o),e.download="contactos.json",e.click()}function v(){const o=new Date().toISOString().slice(0,10);let e=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]");e.find(c=>c.fecha===o)||(e.push({fecha:o,datos:t.contacts}),e.length>10&&(e=e.slice(-10)),localStorage.setItem("contactos_diarios_backups",JSON.stringify(e))),localStorage.setItem("contactos_diarios_backup_fecha",o)}setInterval(v,60*60*1e3);v();function _(){const o=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]"),e=document.getElementById("backup-info");e&&(o.length===0?e.textContent="Sin copias locales.":e.innerHTML="Últimas copias locales: "+o.map(c=>`<button class="restore-backup-btn" data-fecha="${c.fecha}">${c.fecha}</button>`).join(" "))}function q(o){if(!confirm("¿Seguro que quieres restaurar la copia de seguridad del "+o+"? Se sobrescribirán los contactos actuales."))return;const c=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]").find(l=>l.fecha===o);c?(t.contacts=c.datos,p(t.contacts),d(),alert("Backup restaurado correctamente.")):alert("No se encontró la copia seleccionada.")}document.addEventListener("DOMContentLoaded",()=>{d();let o=null;const e=document.createElement("button");e.textContent="📲 Instalar en tu dispositivo",e.className="add-btn",e.style.display="none",e.style.position="fixed",e.style.bottom="1.5rem",e.style.left="50%",e.style.transform="translateX(-50%)",e.style.zIndex="3000",document.body.appendChild(e),window.addEventListener("beforeinstallprompt",c=>{c.preventDefault(),o=c,e.style.display="block"}),e.addEventListener("click",async()=>{if(o){o.prompt();const{outcome:c}=await o.userChoice;c==="accepted"&&(e.style.display="none"),o=null}}),window.addEventListener("appinstalled",()=>{e.style.display="none"})});
