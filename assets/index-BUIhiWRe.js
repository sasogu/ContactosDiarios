(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const _="0.0.77";(function(){try{const t=localStorage.getItem("app_version");if(t&&t!==_){const s=localStorage.getItem("contactos_diarios"),i=localStorage.getItem("contactos_diarios_backups"),a=localStorage.getItem("contactos_diarios_backup_fecha"),o=localStorage.getItem("contactos_diarios_webdav_config");["app_version","contactos_diarios","contactos_diarios_backups","contactos_diarios_backup_fecha","contactos_diarios_webdav_config"].forEach(m=>{m!=="contactos_diarios"&&localStorage.removeItem(m)}),"caches"in window&&caches.keys().then(m=>{m.forEach(b=>{(b.includes("contactosdiarios")||b.includes("contactos-diarios"))&&caches.delete(b)})}),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(m=>{m.forEach(b=>{b.scope.includes(window.location.origin)&&b.unregister()})}),s&&localStorage.setItem("contactos_diarios",s),i&&localStorage.setItem("contactos_diarios_backups",i),a&&localStorage.setItem("contactos_diarios_backup_fecha",a),o&&localStorage.setItem("contactos_diarios_webdav_config",o),location.reload()}localStorage.setItem("app_version",_)}catch{}})();function H({contacts:n,filter:t,onSelect:s,onDelete:i}){let a=t?n.filter(o=>{const d=t.toLowerCase(),m=o.notes?Object.values(o.notes).join(" ").toLowerCase():"";return o.tags?.some(b=>b.toLowerCase().includes(d))||o.name?.toLowerCase().includes(d)||o.surname?.toLowerCase().includes(d)||m.includes(d)}):n;return a=a.slice().sort((o,d)=>d.pinned&&!o.pinned?1:o.pinned&&!d.pinned?-1:(o.surname||"").localeCompare(d.surname||"")),`
  ...
  `;const a=t==="success"?"✅":t==="error"?"❌":t==="warning"?"⚠️":"ℹ️";i.innerHTML=`${a} ${n}`,s.appendChild(i),setTimeout(()=>{i.style.opacity="1",i.style.transform="translateX(0)"},10);const o=()=>{i.style.opacity="0",i.style.transform="translateX(100%)",setTimeout(()=>{i.parentNode&&i.parentNode.removeChild(i)},300)};i.onclick=o,setTimeout(o,4e3)}function L(n){const t=[];return!n.name||n.name.trim().length===0?t.push("El nombre es obligatorio"):n.name.trim().length<2?t.push("El nombre debe tener al menos 2 caracteres"):n.name.trim().length>50&&t.push("El nombre no puede tener más de 50 caracteres"),n.surname&&n.surname.trim().length>50&&t.push("Los apellidos no pueden tener más de 50 caracteres"),n.phone&&n.phone.trim().length>0&&(/^[\d\s\-\+\(\)\.]{6,20}$/.test(n.phone.trim())||t.push("El teléfono debe contener solo números, espacios y caracteres válidos (6-20 caracteres)")),n.email&&n.email.trim().length>0&&(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n.email.trim())?n.email.trim().length>100&&t.push("El email no puede tener más de 100 caracteres"):t.push("El email debe tener un formato válido")),t}function O(n){const t=[];return!n||n.trim().length===0?(t.push("La nota no puede estar vacía"),t):(n.trim().length>1e3&&t.push("La nota no puede tener más de 1000 caracteres"),n.trim().length<3&&t.push("La nota debe tener al menos 3 caracteres"),/^[\w\s\.\,\;\:\!\?\-\(\)\[\]\"\'\/\@\#\$\%\&\*\+\=\<\>\{\}\|\~\`\ñÑáéíóúÁÉÍÓÚüÜ]*$/.test(n)||t.push("La nota contiene caracteres no permitidos"),t)}const z="contactos_diarios";let e={contacts:ne(),selected:null,notes:"",editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,showAddNoteModal:!1,addNoteContactIndex:null,allNotesPage:1,duplicates:[],showDuplicateModal:!1,showAuthModal:!1,authMode:"login",pendingAction:null};function ne(){try{return JSON.parse(localStorage.getItem(z))||[]}catch{return[]}}function I(n){localStorage.setItem(z,JSON.stringify(n))}function p(){const n=document.querySelector("#app"),t=e.editing!==null?e.contacts[e.editing]:null,s=e.selected!==null?e.contacts[e.selected].notes||{}:{};n.innerHTML=`
    ...
    <div class="version-info">
      <small id="sw-version">Service Worker v${ae()}</small>
    </div>
  `,oe();const i=document.getElementById("show-backup-modal");i&&(i.onclick=()=>{e.showBackupModal=!0,p()});const a=document.getElementById("close-backup-modal");a&&(a.onclick=()=>{e.showBackupModal=!1,p()}),document.querySelectorAll(".add-note-contact").forEach(m=>{m.onclick=b=>{const h=Number(m.dataset.index);if(!D()){e.pendingAction={type:"addNote",contactIndex:h},P()?e.authMode="login":e.authMode="setup",e.showAuthModal=!0,p();return}e.addNoteContactIndex=h,e.showAddNoteModal=!0,p()}});const o=document.getElementById("cancel-add-note");o&&(o.onclick=()=>{e.showAddNoteModal=!1,e.addNoteContactIndex=null,p()}),document.querySelectorAll(".restore-backup-btn").forEach(m=>{m.onclick=()=>ue(m.dataset.fecha)});const d=document.getElementById("restore-local-backup");d&&(d.onclick=restaurarBackupLocal)}let V=null;function oe(){const n=document.getElementById("tag-filter");n&&n.addEventListener("input",c=>{clearTimeout(V),V=setTimeout(()=>{e.tagFilter=n.value,p();const l=document.getElementById("tag-filter");l&&(l.value=e.tagFilter,l.focus(),l.setSelectionRange(e.tagFilter.length,e.tagFilter.length))},300)});const t=document.getElementById("add-contact");t&&(t.onclick=()=>{e.editing=null,e.selected=null,p(),e.editing=e.contacts.length,p()}),document.querySelectorAll(".select-contact").forEach(c=>{c.onclick=l=>{e.selected=Number(c.dataset.index),e.editing=null,p()}}),document.querySelectorAll(".edit-contact").forEach(c=>{c.onclick=l=>{e.editing=Number(c.dataset.index),e.selected=null,p()}}),document.querySelectorAll(".delete-contact").forEach(c=>{c.onclick=l=>{const r=Number(c.dataset.index),u=e.contacts[r],f=u.surname?`${u.surname}, ${u.name}`:u.name;confirm(`¿Estás seguro de eliminar el contacto "${f}"?
...
¿Deseas ver más detalles en la consola del navegador?`)&&console.table(c)}});const y=document.getElementById("cancel-duplicate-resolution");y&&(y.onclick=()=>{e.showDuplicateModal=!1,e.duplicates=[],p()});const w=document.getElementById("apply-resolution");w&&(w.onclick=ge),document.querySelectorAll('input[name^="resolution-"]').forEach(c=>{c.addEventListener("change",()=>{const l=c.name.split("-")[1],r=document.getElementById(`merge-section-${l}`),u=document.getElementById(`individual-section-${l}`);c.value==="merge"?(r.style.display="block",u.style.display="none"):c.value==="select"?(r.style.display="none",u.style.display="block"):(r.style.display="none",u.style.display="none")})}),document.querySelectorAll('.resolution-option input[type="radio"]').forEach(c=>{c.addEventListener("change",()=>{const l=c.name;document.querySelectorAll(`input[name="${l}"]`).forEach(r=>{r.closest(".resolution-option").classList.remove("selected")}),c.closest(".resolution-option").classList.add("selected")})});const M=document.getElementById("unlock-notes-btn");M&&(M.onclick=()=>{e.selected!==null&&(e.pendingAction={type:"showContactNotes",contactIndex:e.selected}),P()?e.authMode="login":e.authMode="setup",e.showAuthModal=!0,p()});const R=document.getElementById("logout-btn");R&&(R.onclick=()=>{ve(),p()});const C=document.getElementById("auth-form");C&&!C.hasAttribute("data-handler-added")&&(C.setAttribute("data-handler-added","true"),C.onsubmit=c=>{c.preventDefault();const l=document.getElementById("auth-password").value.trim();if(!l){g("Por favor, introduce una contraseña","warning");return}if(e.authMode==="setup"){const r=document.getElementById("auth-password-confirm").value.trim();if(l!==r){g("Las contraseñas no coinciden","error");return}if(l.length<4){g("La contraseña debe tener al menos 4 caracteres","warning");return}he(l),S.isAuthenticated=!0,S.sessionExpiry=Date.now()+30*60*1e3,e.showAuthModal=!1,C.reset(),setTimeout(()=>{K()},100),p()}else ye(l)?(e.showAuthModal=!1,C.reset(),p()):(document.getElementById("auth-password").value="",document.getElementById("auth-password").focus())});const T=document.getElementById("cancel-auth");T&&(T.onclick=()=>{e.showAuthModal=!1,e.pendingAction=null,p()});const j=document.getElementById("auth-modal");if(j){j.onclick=r=>{r.target===j&&(e.showAuthModal=!1,e.pendingAction=null,p())};const c=document.getElementById("auth-password");if(c){const r=window.innerWidth<=700?300:100;setTimeout(()=>{c.focus(),window.innerWidth<=700&&c.scrollIntoView({behavior:"smooth",block:"center"})},r)}const l=r=>{r.key==="Escape"&&e.showAuthModal&&(e.showAuthModal=!1,e.pendingAction=null,p())};document.addEventListener("keydown",l)}}function ae(){try{return _||"0.0.77"}catch{return"0.0.77"}}function se(n){const t=[],s=n.split("END:VCARD");for(const i of s){const a=/FN:([^\n]*)/.exec(i)?.[1]?.trim(),o=/N:.*;([^;\n]*)/.exec(i)?.[1]?.trim()||"",d=/TEL.*:(.+)/.exec(i)?.[1]?.trim(),m=/EMAIL.*:(.+)/.exec(i)?.[1]?.trim();a&&t.push({name:a,surname:o,phone:d||"",email:m||"",notes:{},tags:[]})}return t}function ie(n){return n.map(t=>`BEGIN:VCARD
VERSION:3.0
FN:${t.name}
N:${t.surname||""};;;;
TEL:${t.phone||""}
...
`),a+=`
Esta acción no se puede deshacer.`,!confirm(a)){g("Operación cancelada","info");return}try{let o=0,d=0;const m=[];n.forEach(y=>{if(y.type==="merge"){const w=W(y.contacts);e.contacts.push(w),y.contacts.forEach(M=>{m.push(M.originalIndex)}),o++}else y.type==="delete"&&(y.toDelete.forEach(w=>{m.push(w)}),d+=y.toDelete.length)}),[...new Set(m)].sort((y,w)=>w-y).forEach(y=>{y<e.contacts.length&&e.contacts.splice(y,1)}),I(e.contacts);let h="Resolución completada: ";const x=[];o>0&&x.push(`${o} contacto(s) fusionado(s)`),d>0&&x.push(`${d} duplicado(s) eliminado(s)`),h+=x.join(" y "),g(h,"success"),e.showDuplicateModal=!1,e.duplicates=[],p()}catch(o){g("Error al aplicar resolución: "+o.message,"error")}}const q="contactos_diarios_auth";let S={isAuthenticated:!1,sessionExpiry:null};function G(n){let t=0;for(let s=0;s<n.length;s++){const i=n.charCodeAt(s);t=(t<<5)-t+i,t=t&t}return t.toString()}function he(n){const t=G(n);localStorage.setItem(q,t),g("Contraseña establecida correctamente","success")}function be(n){const t=localStorage.getItem(q);return t?G(n)===t:!1}function P(){return localStorage.getItem(q)!==null}function ye(n){return be(n)?(S.isAuthenticated=!0,S.sessionExpiry=Date.now()+30*60*1e3,g("Autenticación exitosa","success"),setTimeout(()=>{K()},100),!0):(g("Contraseña incorrecta","error"),!1)}function D(){return S.isAuthenticated?Date.now()>S.sessionExpiry?(S.isAuthenticated=!1,S.sessionExpiry=null,!1):!0:!1}function ve(){S.isAuthenticated=!1,S.sessionExpiry=null,g("Sesión cerrada","info")}function K(){if(!e.pendingAction)return;const n=e.pendingAction;switch(e.pendingAction=null,n.type){case"showAllNotes":e.showAllNotes=!0,e.allNotesPage=1;break;case"addNote":e.addNoteContactIndex=n.contactIndex,e.showAddNoteModal=!0;break;case"showContactNotes":e.selected=n.contactIndex,e.editing=null;break}p()}function xe({visible:n,mode:t="login"}){return`
