(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function c(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(s){if(s.ep)return;s.ep=!0;const n=c(s);fetch(s.href,n)}})();const v="0.0.24";(function(){try{const e=localStorage.getItem("app_version");if(e&&e!==v){const c=localStorage.getItem("contactos_diarios");Object.keys(localStorage).forEach(i=>{i!=="contactos_diarios"&&localStorage.removeItem(i)}),"caches"in window&&caches.keys().then(i=>i.forEach(s=>caches.delete(s))),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.getRegistrations().then(i=>i.forEach(s=>s.unregister())),c&&localStorage.setItem("contactos_diarios",c),location.reload()}localStorage.setItem("app_version",v)}catch{}})();function I({contacts:o,filter:e,onSelect:c,onDelete:i}){let s=e?o.filter(n=>{const d=e.toLowerCase(),f=n.notes?Object.values(n.notes).join(" ").toLowerCase():"";return n.tags?.some(l=>l.toLowerCase().includes(d))||n.name?.toLowerCase().includes(d)||n.surname?.toLowerCase().includes(d)||f.includes(d)}):o;return s=s.slice().sort((n,d)=>d.pinned&&!n.pinned?1:n.pinned&&!d.pinned?-1:(n.surname||"").localeCompare(d.surname||"")),`
  ...
  `}function B({contacts:o,visible:e,page:c=1}){let i=[];o.forEach((a,r)=>{a.notes&&Object.entries(a.notes).forEach(([m,p])=>{i.push({date:m,text:p,contact:a,contactIndex:r})})}),i.sort((a,r)=>r.date.localeCompare(a.date));const s=4,n=Math.max(1,Math.ceil(i.length/s)),d=Math.min(Math.max(c,1),n),f=(d-1)*s,l=i.slice(f,f+s);return`
  ...
  `}function w({visible:o,backups:e}){return`
  ...
  `}const x="contactos_diarios";let t={contacts:O(),selected:null,editing:null,tagFilter:"",showAllNotes:!1,showBackupModal:!1,allNotesPage:1};function O(){try{return JSON.parse(localStorage.getItem(x))||[]}catch{return[]}}function g(o){localStorage.setItem(x,JSON.stringify(o))}function u(){const o=document.querySelector("#app"),e=t.editing!==null?t.contacts[t.editing]:null,c=t.selected!==null?t.contacts[t.selected].notes||{}:{};o.innerHTML=`
...
EMAIL:${e.email||""}
END:VCARD`).join(`
`)}function L(o){const e=o.split(`
`).filter(Boolean),[c,...i]=e;return i.map(s=>{const[n,d,f,l,a,r]=s.split(",");return{name:n?.trim()||"",surname:d?.trim()||"",phone:f?.trim()||"",email:l?.trim()||"",notes:r?JSON.parse(r):{},tags:a?a.split(";").map(m=>m.trim()):[]}})}function P(){const o=A(t.contacts),e=new Blob([o],{type:"text/vcard"}),c=document.createElement("a");c.href=URL.createObjectURL(e),c.download="contactos.vcf",c.click()}function _(){const o="name,surname,phone,email,tags,notes",e=t.contacts.map(n=>[n.name,n.surname,n.phone,n.email,(n.tags||[]).join(";"),JSON.stringify(n.notes||{})].map(d=>'"'+String(d).replace(/"/g,'""')+'"').join(",")),c=[o,...e].join(`
`),i=new Blob([c],{type:"text/csv"}),s=document.createElement("a");s.href=URL.createObjectURL(i),s.download="contactos.csv",s.click()}function q(){const o=new Blob([JSON.stringify(t.contacts,null,2)],{type:"application/json"}),e=document.createElement("a");e.href=URL.createObjectURL(o),e.download="contactos.json",e.click()}function k(){const o=new Date().toISOString().slice(0,10);let e=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]");e.find(c=>c.fecha===o)||(e.push({fecha:o,datos:t.contacts}),e.length>10&&(e=e.slice(-10)),localStorage.setItem("contactos_diarios_backups",JSON.stringify(e))),localStorage.setItem("contactos_diarios_backup_fecha",o)}setInterval(k,60*60*1e3);k();function M(){const o=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]"),e=document.getElementById("backup-info");e&&(o.length===0?e.textContent="Sin copias locales.":e.innerHTML="Últimas copias locales: "+o.map(c=>`<button class="restore-backup-btn" data-fecha="${c.fecha}">${c.fecha}</button>`).join(" "))}function D(o){if(!confirm("¿Seguro que quieres restaurar la copia de seguridad del "+o+"? Se sobrescribirán los contactos actuales."))return;const c=JSON.parse(localStorage.getItem("contactos_diarios_backups")||"[]").find(i=>i.fecha===o);c?(t.contacts=c.datos,g(t.contacts),u(),alert("Backup restaurado correctamente.")):alert("No se encontró la copia seleccionada.")}document.addEventListener("DOMContentLoaded",()=>{u();let o=null;const e=document.createElement("button");e.textContent="📲 Instalar en tu dispositivo",e.className="add-btn",e.style.display="none",e.style.position="fixed",e.style.bottom="1.5rem",e.style.left="50%",e.style.transform="translateX(-50%)",e.style.zIndex="3000",document.body.appendChild(e),window.addEventListener("beforeinstallprompt",c=>{c.preventDefault(),o=c,e.style.display="block"}),e.addEventListener("click",async()=>{if(o){o.prompt();const{outcome:c}=await o.userChoice;c==="accepted"&&(e.style.display="none"),o=null}}),window.addEventListener("appinstalled",()=>{e.style.display="none"})});
